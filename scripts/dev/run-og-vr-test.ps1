...
if ([string]::IsNullOrWhiteSpace($ProofPath)) {
    $ProofPath = Join-Path (Get-NikamiProofRoot -WorkspaceRoot $WorkspaceRoot) "og_vr_current_test.log"
}
...
$env:SWG_OG_VR = "1"
$env:XR_RUNTIME_JSON = $oculusRuntimeJson
...
$env:SWG_OG_VR_DETACHED_HANDS_REQUIRE_TRACKED = "1"
...
Write-Output "Launching OG VR test:"
Write-Output "  ClientDataRoot=$ClientDataRoot"
Write-Output "  ProofPath=$(if ($ProofTrace) { $ProofPath } else { 'off' })"
Write-Output "  HandTracePath=off"
...
Write-Output "  Overhead target UI=on only as a source for object-context target/status windows"
Write-Output "  Grip wand=$(if ($wandVisualsEnabled) { 'visible' } else { 'hidden' })"
...
Write-Output "  OpenXR runtime=Oculus JSON; explicit loader=$oculusOpenXrLoader"
Write-Output "  Body IK hand proof marker=$handBuildMarker"
Write-Output "  Renderer hand layer marker=$rendererBuildMarker"
Write-Output "  VR weapon inventory=A/B cycles real inventory weapons, including empty hands"
Write-Output "  Static set dressing physics=on"
Write-Output "  TV mode=$(if ($TvMode) { 'forced on' } else { 'default startup quad, then world mode after game entry' })"
