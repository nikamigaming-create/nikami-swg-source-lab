...
namespace SwgCuiHudWindowManagerNamespace
{
	...
	bool isDeadCreatureInventory(ClientObject const &container)
	{
		ClientObject const * const containedBy = safe_cast<ClientObject const *>(ContainerInterface::getContainedByObject(container));
		CreatureObject const * const creature = containedBy ? containedBy->asCreatureObject() : 0;
		CreatureObject const * const player = Game::getPlayerCreature();
		return creature && creature->isDead() && creature != player;
	}

	void positionVrStockInventoryWindow(SwgCuiInventory &inventory, ClientObject const *container)
	{
		if (!container ||
			!environmentEnabledDefault("SWG_OG_VR", false) ||
			!environmentEnabledDefault("SWG_OG_VR_STOCK_INVENTORY_PANEL_PLACEMENT", true))
		{
			return;
		}

		int const frameWidth = Graphics::getFrameBufferMaxWidth();
		int const frameHeight = Graphics::getFrameBufferMaxHeight();
		if (frameWidth <= 0 || frameHeight <= 0)
			return;

		if (isDeadCreatureInventory(*container))
		{
			int const width = 400;
			int const height = 245;
			int const x = clampInt((frameWidth / 2) - width - 18, 16, frameWidth - width - 16);
			int const y = clampInt((frameHeight / 2) - (height / 2), 16, frameHeight - height - 16);
			inventory.getPage().SetLocation(x, y);
		}
		else if (CuiInventoryManager::isPlayerInventory(*container))
		{
			int const width = 688;
			int const height = 519;
			int const x = clampInt((frameWidth / 2) + 18, 16, frameWidth - width - 16);
			int const y = clampInt((frameHeight / 2) - (height / 2), 16, frameHeight - height - 16);
			inventory.getPage().SetLocation(x, y);
		}
	}
...
SwgCuiInventory * SwgCuiHudWindowManager::createInventory (ClientObject * container, const std::string & slotName, bool usePaperDoll)
{
	...
	{
		inv->getPage ().Center ();
		positionVrStockInventoryWindow(*inv, container);
	...
	else
	{
		...
		if (!m_pendingItemOpenInfo->empty ())
		{
			for (ItemRequestInfoVector::iterator it = m_pendingItemOpenInfo->begin (); it != m_pendingItemOpenInfo->end (); ++it)
			{
				...
				positionVrStockInventoryWindow(*inventory, container);
