...
namespace SwgCuiButtonBarNamespace
{
	...
	bool getEnvFlagDefault(char const * const name, bool const defaultValue)
	{
		char const * const value = getenv(name);
		if (!value || !*value)
			return defaultValue;

		if (_stricmp(value, "0") == 0 || _stricmp(value, "false") == 0 || _stricmp(value, "no") == 0 || _stricmp(value, "off") == 0)
			return false;
	...
	float getVrAwareMenuButtonOpacity()
	{
		bool const vrEnabled =
			getEnvFlagDefault("SWG_OG_VR", false) ||
			getEnvFlagDefault("SWG_D3D11_VR", false) ||
			getEnvFlagDefault("SWG_OG_VR_ENABLE", false);
		return vrEnabled ? 1.0f : CuiPreferences::getCommandButtonOpacity();
	}
...
m_opacityCallback      (0)
{
	...
	m_menuButtonPage->SetOpacity(getVrAwareMenuButtonOpacity());
