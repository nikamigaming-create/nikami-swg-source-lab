...
namespace
{
	...
	bool s_envAutoSelectedAvatar = false;
	int s_envAutoSelectFrames = 0;
	...
	bool shouldEnvAutoSelectAvatar ()
	{
		char const * const value = std::getenv ("SWG_OG_AUTO_SELECT_AVATAR");
		return value && *value && _stricmp (value, "0") != 0 && _stricmp (value, "false") != 0 && _stricmp (value, "no") != 0;
	}
...
void SwgCuiAvatarSelection::performDeactivate ()
{
	...
	disconnectAll();
	VrDetachedHands::setMenuPreviewCreature(0);
...
void SwgCuiAvatarSelection::updateAvatarSelection ()
{
	...
	if (row >= 0 && cellData)
	{
		...
		else
		{
			...
			VrDetachedHands::setMenuPreviewCreature(avatar);
...
void SwgCuiAvatarSelection::update (float deltaTimeSecs)
{
	...
	if (shouldEnvAutoSelectAvatar () && !s_envAutoSelectedAvatar)
	{
		UITableModelDefault * const model = NON_NULL (safe_cast<UITableModelDefault *>(m_table->GetTableModel ()));
		if (model->GetRowCount () > 0)
		{
			...
			if (m_updateAvatar <= 0)
			{
				if (++s_envAutoSelectFrames > 8)
				{
					updateAvatarSelection ();
					if (getCurrentlySelectedAvatar ())
					{
						DEBUG_REPORT_LOG (true, ("SWG_OG_AUTO_SELECT_AVATAR requesting selected avatar\n"));
						s_envAutoSelectedAvatar = true;
						requestAvatarSelection ();
					}
				}
			}
