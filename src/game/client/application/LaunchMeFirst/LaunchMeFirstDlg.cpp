...
namespace
{
	...
	{
		DWORD const attributes = GetFileAttributes(path);
		return attributes != INVALID_FILE_ATTRIBUTES && (attributes & FILE_ATTRIBUTE_DIRECTORY) == 0;
	}
}

bool CLaunchMeFirstDlg::runModernClient(bool vr)
{
	char const * const modernRoot = "D:\\code\\swg\\modern-client";
	char const * const clientDataRoot = "D:\\code\\swg\\client\\SWGSource Client v3.0";
	char modernExe[MAX_PATH];

	strcpy(modernExe, "D:\\code\\swg\\modern-client\\build\\Release\\swg_modern_client.exe");
	if (!fileExists(modernExe))
	{
		strcpy(modernExe, "D:\\code\\swg\\modern-client\\build\\RelWithDebInfo\\swg_modern_client.exe");
		if (!fileExists(modernExe))
		{
			strcpy(modernExe, "D:\\code\\swg\\modern-client\\build\\Debug\\swg_modern_client.exe");
		}
	}
	...
	{
		CString message;
		message.Format("Modern client executable not found:\n%s", modernExe);
	...
	sprintf(commandLine, "\"%s\" \"%s\"%s", modernExe, clientDataRoot, vr ? " --vr" : "");

	BOOL result = CreateProcess(modernExe, commandLine, NULL, NULL, FALSE, 0, NULL, modernRoot, &si, &pi);
	if (!result)
		MessageBox("Failed to start modern client", NULL, MB_ICONERROR | MB_OK);
	else
	{
...
void CLaunchMeFirstDlg::OnOK() 
{
	...
	switch (m_radio)
	{
		...
		case 8:
			result = runModernClient(false);
			break;

		case 9:
			result = runModernClient(true);
			break;
	}
