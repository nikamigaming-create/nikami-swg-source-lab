...
#if defined(_WIN64)
	void               setOnErase    (ObjectType * owner, void (ObjectType::*onErase)(const size_t, const ValueType &));
	void               setOnInsert   (ObjectType * owner, void (ObjectType::*onInsert)(const size_t, const ValueType &));
	void               setOnSet      (ObjectType * owner, void (ObjectType::*onSet)(const size_t, const ValueType &, const ValueType &));
#endif
...
#if defined(_WIN64)
	std::pair<ObjectType *, void (ObjectType::*)(const size_t, const ValueType &)> * onEraseCallbackSz;
	std::pair<ObjectType *, void (ObjectType::*)(const size_t, const ValueType &)> * onInsertCallbackSz;
	std::pair<ObjectType *, void (ObjectType::*)(const size_t, const ValueType &, const ValueType &)> * onSetCallbackSz;
#endif
...
#if defined(_WIN64)
	delete onEraseCallbackSz;
	delete onInsertCallbackSz;
	delete onSetCallbackSz;
#endif
}
...
#if defined(_WIN64)
	else if(onEraseCallbackSz)
	{
		if(onEraseCallbackSz->first)
		{
			ObjectType & owner = *onEraseCallbackSz->first;
			void (ObjectType::*cb)(const size_t, const ValueType &) = onEraseCallbackSz->second;
			(owner.*cb)(static_cast<size_t>(element), oldValue);
		}
	}
#endif
}
...
#if defined(_WIN64)
	else if(onInsertCallbackSz)
	{
		if(onInsertCallbackSz->first)
		{
			ObjectType & owner = *onInsertCallbackSz->first;
			void (ObjectType::*cb)(const size_t, const ValueType &) = onInsertCallbackSz->second;
			(owner.*cb)(static_cast<size_t>(element), newValue);
		}
	}
#endif
}
...
#if defined(_WIN64)
	else if(onSetCallbackSz)
	{
		if(onSetCallbackSz->first)
		{
			ObjectType & owner = *onSetCallbackSz->first;
			void (ObjectType::*cb)(const size_t, const ValueType &, const ValueType &) = onSetCallbackSz->second;
			(owner.*cb)(static_cast<size_t>(element), oldValue, newValue);
		}
	}
#endif
}
...
//-----------------------------------------------------------------------
template<typename ValueType, typename ObjectType>
inline void AutoDeltaVector<ValueType, ObjectType>::setOnErase(ObjectType * owner, void (ObjectType::*cb)(const size_t, const ValueType &))
{
	delete onEraseCallbackSz;
	onEraseCallbackSz = new std::pair<ObjectType *, void (ObjectType::*)(const size_t, const ValueType &)>;
	onEraseCallbackSz->first = owner;
	onEraseCallbackSz->second = cb;
}

//-----------------------------------------------------------------------
template<typename ValueType, typename ObjectType>
inline void AutoDeltaVector<ValueType, ObjectType>::setOnInsert(ObjectType * owner, void (ObjectType::*cb)(const size_t, const ValueType &))
{
	delete onInsertCallbackSz;
	onInsertCallbackSz = new std::pair<ObjectType *, void (ObjectType::*)(const size_t, const ValueType &)>;
	onInsertCallbackSz->first = owner;
	onInsertCallbackSz->second = cb;
}

//-----------------------------------------------------------------------
template<typename ValueType, typename ObjectType>
inline void AutoDeltaVector<ValueType, ObjectType>::setOnSet(ObjectType * owner, void (ObjectType::*cb)(const size_t, const ValueType &, const ValueType &))
{
	delete onSetCallbackSz;
	onSetCallbackSz = new std::pair<ObjectType *, void (ObjectType::*)(const size_t, const ValueType &, const ValueType &)>;
	onSetCallbackSz->first = owner;
	onSetCallbackSz->second = cb;
}
#endif
...
inline void AutoDeltaVector<ValueType, ObjectType>::unpack(ReadIterator & source) 
{
	...
	unsigned int packedBaselineCommandCount;
	ValueType value;

	Archive::get(source, commandCount);
	Archive::get(source, packedBaselineCommandCount);
	baselineCommandCount = packedBaselineCommandCount;
...
inline void AutoDeltaVector<ValueType, ObjectType>::unpackDelta(ReadIterator & source)
{
	...
	for ( ; i < static_cast<size_t>(commandCount); ++i)
	{
		Archive::get(source, c.cmd);
		switch (c.cmd)
		{
