...
public:
	...
	static Direct3d9_TextureData const *getTexture(int stage);
	static DWORD getTextureStageState(int stage, D3DTEXTURESTAGESTATETYPE state);
...
inline Direct3d9_TextureData const *Direct3d9_StateCache::getTexture(int stage)
{
	DEBUG_FATAL(stage < 0 || stage >= cms_samplers, ("Sampler out of range %d/%d", stage, cms_samplers));
	return ms_textureCache[stage];
}

// ----------------------------------------------------------------------

inline DWORD Direct3d9_StateCache::getTextureStageState(int stage, D3DTEXTURESTAGESTATETYPE state)
{
	DEBUG_FATAL(stage < 0 || stage >= cms_stages, ("stage out of range %d/%d", stage, cms_stages));
	return ms_textureStagesStateCache[stage][state];
}
