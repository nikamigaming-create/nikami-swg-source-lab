...
//	VectorRgba const &                 getAlphaFadeAndBloomSettings();
	...
	int                                        ms_fogLogCount;
	bool                                       ms_fogEnabled;
	int                                        ms_activeFogMode;
	D3DMATERIAL9                               ms_activeInventoryMaterial;
	VectorRgba                                 ms_activeInventoryTextureFactor;
	VectorRgba                                 ms_activeInventoryTextureFactor2;
	...
	DWORD                                      ms_fogColor;
	DWORD                                      ms_activeFogColor;
	DWORD                                      ms_fogModeValue;

	bool                                       ms_antialiasEnabled = false;

	void setDefaultInventoryMaterialAndFactor()
	{
		ms_activeInventoryMaterial.Ambient.r  = 1.0f;
		ms_activeInventoryMaterial.Ambient.g  = 1.0f;
		ms_activeInventoryMaterial.Ambient.b  = 1.0f;
		ms_activeInventoryMaterial.Ambient.a  = 1.0f;
		ms_activeInventoryMaterial.Diffuse.r  = 1.0f;
		ms_activeInventoryMaterial.Diffuse.g  = 1.0f;
		ms_activeInventoryMaterial.Diffuse.b  = 1.0f;
		ms_activeInventoryMaterial.Diffuse.a  = 1.0f;
		ms_activeInventoryMaterial.Emissive.r = 0.0f;
		ms_activeInventoryMaterial.Emissive.g = 0.0f;
		ms_activeInventoryMaterial.Emissive.b = 0.0f;
		ms_activeInventoryMaterial.Emissive.a = 0.0f;
		ms_activeInventoryMaterial.Specular.r = 0.0f;
		ms_activeInventoryMaterial.Specular.g = 0.0f;
		ms_activeInventoryMaterial.Specular.b = 0.0f;
		ms_activeInventoryMaterial.Specular.a = 0.0f;
		ms_activeInventoryMaterial.Power      = 32.0f;
		ms_activeInventoryTextureFactor.r     = 1.0f;
		ms_activeInventoryTextureFactor.g     = 1.0f;
		ms_activeInventoryTextureFactor.b     = 1.0f;
		ms_activeInventoryTextureFactor.a     = 1.0f;
		ms_activeInventoryTextureFactor2.r    = 1.0f;
		ms_activeInventoryTextureFactor2.g    = 1.0f;
		ms_activeInventoryTextureFactor2.b    = 1.0f;
		ms_activeInventoryTextureFactor2.a    = 1.0f;
	}
	...
	{
		if (ms_rendererInventoryEnabled < 0)
		{
			char enabled[16];
			DWORD const length = GetEnvironmentVariableA("SWG_RENDERER_INVENTORY", enabled, sizeof(enabled));
			ms_rendererInventoryEnabled = (length > 0 && length < sizeof(enabled) && enabled[0] != '0') ? 1 : 0;
		}

		return ms_rendererInventoryEnabled != 0;
	}

	int getRendererInventoryLimit()
	{
		if (ms_rendererInventoryLimit < 0)
		{
			char limitText[32];
			DWORD const length = GetEnvironmentVariableA("SWG_RENDERER_INVENTORY_LIMIT", limitText, sizeof(limitText));
			ms_rendererInventoryLimit = (length > 0 && length < sizeof(limitText)) ? atoi(limitText) : 50000;
			if (ms_rendererInventoryLimit <= 0)
				ms_rendererInventoryLimit = 0x7fffffff;
		}

		return ms_rendererInventoryLimit;
	}

	int getRendererInventoryStartFrame()
	{
		if (ms_rendererInventoryStartFrame < 0)
		{
			char startText[32];
			DWORD const length = GetEnvironmentVariableA("SWG_RENDERER_INVENTORY_START_FRAME", startText, sizeof(startText));
			ms_rendererInventoryStartFrame = (length > 0 && length < sizeof(startText)) ? atoi(startText) : 0;
			if (ms_rendererInventoryStartFrame < 0)
				ms_rendererInventoryStartFrame = 0;
		}

		return ms_rendererInventoryStartFrame;
	}

	void rendererInventoryDiag(char const *format, ...);

	int getAutoCaptureWorldRowsThreshold()
	{
		if (ms_autoCaptureWorldRowsThreshold < 0)
		{
			...
			ms_autoCaptureWorldRowsThreshold = (length > 0 && length < sizeof(thresholdText)) ? atoi(thresholdText) : 80;
			if (ms_autoCaptureWorldRowsThreshold < 1)
				ms_autoCaptureWorldRowsThreshold = 1;
		}

		return ms_autoCaptureWorldRowsThreshold;
	}

	int getRendererAutoCaptureInt(char const *name, int defaultValue, int minimumValue)
	{
		char text[32];
		DWORD const length = GetEnvironmentVariableA(name, text, sizeof(text));
		int value = (length > 0 && length < sizeof(text)) ? atoi(text) : defaultValue;
	...
	{
		if (ms_autoCaptureDone || ms_autoCaptureRequested || isUiOrLoadingShaderName(ms_activeStaticShaderName))
			return;

		char captureBase[MAX_PATH];
		DWORD const length = GetEnvironmentVariableA("SWG_D3D9_AUTOCAPTURE", captureBase, sizeof(captureBase));
		if (length == 0 || length >= sizeof(captureBase))
			return;

		++ms_autoCaptureWorldRows;
		if (ms_autoCaptureWorldRows >= getAutoCaptureWorldRowsThreshold())
		{
			ms_autoCaptureRequested = true;
			rendererInventoryDiag("d3d9 autocapture armed frame=%d worldRows=%d shader=%s", ms_presentCount, ms_autoCaptureWorldRows, ms_activeStaticShaderName);
		}
	}
	...
	void logRendererInventoryDraw(char const *drawKind, D3DPRIMITIVETYPE primitiveType, int baseIndex, int startIndex, int primitiveCount, int numberOfIndices)
	{
		if (!isRendererInventoryEnabled() || ms_presentCount < getRendererInventoryStartFrame() || ms_rendererInventoryLogCount >= getRendererInventoryLimit())
			return;

		Direct3d9_TextureData const *texture0 = Direct3d9_StateCache::getTexture(0);
		Direct3d9_TextureData const *texture1 = Direct3d9_StateCache::getTexture(1);
		Direct3d9_TextureData const *texture2 = Direct3d9_StateCache::getTexture(2);
		DWORD const texCoord0 = Direct3d9_StateCache::getTextureStageState(0, D3DTSS_TEXCOORDINDEX);
		DWORD const texCoord1 = Direct3d9_StateCache::getTextureStageState(1, D3DTSS_TEXCOORDINDEX);
		DWORD const texCoord2 = Direct3d9_StateCache::getTextureStageState(2, D3DTSS_TEXCOORDINDEX);
		DWORD const s0ColorOp = Direct3d9_StateCache::getTextureStageState(0, D3DTSS_COLOROP);
		DWORD const s0ColorArg0 = Direct3d9_StateCache::getTextureStageState(0, D3DTSS_COLORARG0);
		DWORD const s0ColorArg1 = Direct3d9_StateCache::getTextureStageState(0, D3DTSS_COLORARG1);
		DWORD const s0ColorArg2 = Direct3d9_StateCache::getTextureStageState(0, D3DTSS_COLORARG2);
		DWORD const s0AlphaOp = Direct3d9_StateCache::getTextureStageState(0, D3DTSS_ALPHAOP);
		DWORD const s0AlphaArg0 = Direct3d9_StateCache::getTextureStageState(0, D3DTSS_ALPHAARG0);
		DWORD const s0AlphaArg1 = Direct3d9_StateCache::getTextureStageState(0, D3DTSS_ALPHAARG1);
		DWORD const s0AlphaArg2 = Direct3d9_StateCache::getTextureStageState(0, D3DTSS_ALPHAARG2);
		DWORD const s0ResultArg = Direct3d9_StateCache::getTextureStageState(0, D3DTSS_RESULTARG);
		DWORD const s1ColorOp = Direct3d9_StateCache::getTextureStageState(1, D3DTSS_COLOROP);
		DWORD const s1ColorArg0 = Direct3d9_StateCache::getTextureStageState(1, D3DTSS_COLORARG0);
		DWORD const s1ColorArg1 = Direct3d9_StateCache::getTextureStageState(1, D3DTSS_COLORARG1);
		DWORD const s1ColorArg2 = Direct3d9_StateCache::getTextureStageState(1, D3DTSS_COLORARG2);
		DWORD const s1AlphaOp = Direct3d9_StateCache::getTextureStageState(1, D3DTSS_ALPHAOP);
		DWORD const s1AlphaArg0 = Direct3d9_StateCache::getTextureStageState(1, D3DTSS_ALPHAARG0);
		DWORD const s1AlphaArg1 = Direct3d9_StateCache::getTextureStageState(1, D3DTSS_ALPHAARG1);
		DWORD const s1AlphaArg2 = Direct3d9_StateCache::getTextureStageState(1, D3DTSS_ALPHAARG2);
		DWORD const s1ResultArg = Direct3d9_StateCache::getTextureStageState(1, D3DTSS_RESULTARG);
		DWORD const s2ColorOp = Direct3d9_StateCache::getTextureStageState(2, D3DTSS_COLOROP);
		DWORD const s2ColorArg0 = Direct3d9_StateCache::getTextureStageState(2, D3DTSS_COLORARG0);
		DWORD const s2ColorArg1 = Direct3d9_StateCache::getTextureStageState(2, D3DTSS_COLORARG1);
		DWORD const s2ColorArg2 = Direct3d9_StateCache::getTextureStageState(2, D3DTSS_COLORARG2);
		DWORD const s2AlphaOp = Direct3d9_StateCache::getTextureStageState(2, D3DTSS_ALPHAOP);
		DWORD const s2AlphaArg0 = Direct3d9_StateCache::getTextureStageState(2, D3DTSS_ALPHAARG0);
		DWORD const s2AlphaArg1 = Direct3d9_StateCache::getTextureStageState(2, D3DTSS_ALPHAARG1);
		DWORD const s2AlphaArg2 = Direct3d9_StateCache::getTextureStageState(2, D3DTSS_ALPHAARG2);
		DWORD const s2ResultArg = Direct3d9_StateCache::getTextureStageState(2, D3DTSS_RESULTARG);
		float lightAmbient[4];
		float lightDiffuse[4];
		float lightDirection[4];
		int enabledLights = 0;
		Direct3d9_LightManager::getInventoryLighting(lightAmbient, lightDiffuse, lightDirection, enabledLights);
		++ms_rendererInventoryLogCount;
		rendererInventoryDiag("renderer inventory d3d9 frame=%d draw=%04d kind=%s prim=%d pass=%d base=%d start=%d primCount=%d indices=%d shader=%s pprogram=%s alphaBlend=%d alphaFade=%d alphaFadeOpacity=%0.3f src=%u dst=%u op=%u alphaTest=%u cmp=%u alphaRef=%u depth=%u write=%u colorWrite=0x%02x fog=%d passFog=%d fogColor=0x%08x sliceVerts=%d sliceIndices=%d s0fmt=%d s0tci=%u s1fmt=%d s1tci=%u s2fmt=%d s2tci=%u s0cop=%u s0c0=%u s0c1=%u s0c2=%u s0aop=%u s0a0=%u s0a1=%u s0a2=%u s0res=%u s1cop=%u s1c0=%u s1c1=%u s1c2=%u s1aop=%u s1a0=%u s1a1=%u s1a2=%u s1res=%u s2cop=%u s2c0=%u s2c1=%u s2c2=%u s2aop=%u s2a0=%u s2a1=%u s2a2=%u s2res=%u lightAmb=%0.3f,%0.3f,%0.3f lightDiff=%0.3f,%0.3f,%0.3f lightDir=%0.3f,%0.3f,%0.3f enabledLights=0x%04x matA=%0.3f,%0.3f,%0.3f matD=%0.3f,%0.3f,%0.3f matE=%0.3f,%0.3f,%0.3f matS=%0.3f,%0.3f,%0.3f specPower=%0.3f tf=%0.3f,%0.3f,%0.3f,%0.3f tf2=%0.3f,%0.3f,%0.3f,%0.3f",
			ms_presentCount,
			ms_rendererInventoryLogCount,
			drawKind,
			static_cast<int>(primitiveType),
			ms_activeStaticShaderPass,
			baseIndex,
			startIndex,
			primitiveCount,
			numberOfIndices,
			ms_activeStaticShaderName,
			Direct3d9_ShaderImplementationData::getActivePixelProgramName(),
			ms_alphaBlendEnable ? 1 : 0,
			ms_alphaFadeOpacityEnabled ? 1 : 0,
			ms_alphaFadeOpacity.a,
			static_cast<unsigned>(Direct3d9_ShaderImplementationData::getActiveAlphaBlendSource()),
			static_cast<unsigned>(Direct3d9_ShaderImplementationData::getActiveAlphaBlendDestination()),
			static_cast<unsigned>(Direct3d9_ShaderImplementationData::getActiveAlphaBlendOperation()),
			static_cast<unsigned>(Direct3d9_ShaderImplementationData::getActiveAlphaTestEnable()),
			static_cast<unsigned>(Direct3d9_ShaderImplementationData::getActiveAlphaTestFunction()),
			static_cast<unsigned>(ms_alphaTestReferenceValue),
			static_cast<unsigned>(Direct3d9_ShaderImplementationData::getActiveZEnable()),
			static_cast<unsigned>(Direct3d9_ShaderImplementationData::getActiveZWrite()),
			static_cast<unsigned>(ms_colorWriteEnable),
			ms_fogEnabled ? 1 : 0,
			ms_activeFogMode,
			static_cast<unsigned>(ms_activeFogColor),
			ms_sliceNumberOfVertices,
			ms_sliceNumberOfIndices,
			texture0 ? static_cast<int>(texture0->getNativeFormat()) : -1,
			static_cast<unsigned>(texCoord0),
			texture1 ? static_cast<int>(texture1->getNativeFormat()) : -1,
			static_cast<unsigned>(texCoord1),
			texture2 ? static_cast<int>(texture2->getNativeFormat()) : -1,
			static_cast<unsigned>(texCoord2),
			static_cast<unsigned>(s0ColorOp),
			static_cast<unsigned>(s0ColorArg0),
			static_cast<unsigned>(s0ColorArg1),
			static_cast<unsigned>(s0ColorArg2),
			static_cast<unsigned>(s0AlphaOp),
			static_cast<unsigned>(s0AlphaArg0),
			static_cast<unsigned>(s0AlphaArg1),
			static_cast<unsigned>(s0AlphaArg2),
			static_cast<unsigned>(s0ResultArg),
			static_cast<unsigned>(s1ColorOp),
			static_cast<unsigned>(s1ColorArg0),
			static_cast<unsigned>(s1ColorArg1),
			static_cast<unsigned>(s1ColorArg2),
			static_cast<unsigned>(s1AlphaOp),
			static_cast<unsigned>(s1AlphaArg0),
			static_cast<unsigned>(s1AlphaArg1),
			static_cast<unsigned>(s1AlphaArg2),
			static_cast<unsigned>(s1ResultArg),
			static_cast<unsigned>(s2ColorOp),
			static_cast<unsigned>(s2ColorArg0),
			static_cast<unsigned>(s2ColorArg1),
			static_cast<unsigned>(s2ColorArg2),
			static_cast<unsigned>(s2AlphaOp),
			static_cast<unsigned>(s2AlphaArg0),
			static_cast<unsigned>(s2AlphaArg1),
			static_cast<unsigned>(s2AlphaArg2),
			static_cast<unsigned>(s2ResultArg),
			lightAmbient[0],
			lightAmbient[1],
			lightAmbient[2],
			lightDiffuse[0],
			...
			lightDirection[1],
			lightDirection[2],
			static_cast<unsigned>(enabledLights),
			ms_activeInventoryMaterial.Ambient.r,
			ms_activeInventoryMaterial.Ambient.g,
			ms_activeInventoryMaterial.Ambient.b,
			ms_activeInventoryMaterial.Diffuse.r,
			ms_activeInventoryMaterial.Diffuse.g,
			ms_activeInventoryMaterial.Diffuse.b,
			ms_activeInventoryMaterial.Emissive.r,
			ms_activeInventoryMaterial.Emissive.g,
			ms_activeInventoryMaterial.Emissive.b,
			ms_activeInventoryMaterial.Specular.r,
			ms_activeInventoryMaterial.Specular.g,
			ms_activeInventoryMaterial.Specular.b,
			ms_activeInventoryMaterial.Power,
			ms_activeInventoryTextureFactor.r,
			ms_activeInventoryTextureFactor.g,
			ms_activeInventoryTextureFactor.b,
			ms_activeInventoryTextureFactor.a,
			ms_activeInventoryTextureFactor2.r,
			ms_activeInventoryTextureFactor2.g,
			ms_activeInventoryTextureFactor2.b,
			ms_activeInventoryTextureFactor2.a);
...
void Direct3d9::setActiveFogColor(DWORD color)
{
	ms_activeFogColor = color;
}

// ----------------------------------------------------------------------

void Direct3d9::setActiveFogMode(int fogMode)
{
	ms_activeFogMode = fogMode;
}

// ----------------------------------------------------------------------

void Direct3d9::setActiveMaterialForInventory(D3DMATERIAL9 const &material)
{
	ms_activeInventoryMaterial = material;
}

// ----------------------------------------------------------------------

void Direct3d9::setActiveTextureFactorForInventory(DWORD textureFactor)
{
	ms_activeInventoryTextureFactor.r = static_cast<float>((textureFactor >> 16) & 0xff) / 255.0f;
	ms_activeInventoryTextureFactor.g = static_cast<float>((textureFactor >>  8) & 0xff) / 255.0f;
	ms_activeInventoryTextureFactor.b = static_cast<float>((textureFactor >>  0) & 0xff) / 255.0f;
	ms_activeInventoryTextureFactor.a = static_cast<float>((textureFactor >> 24) & 0xff) / 255.0f;
	ms_activeInventoryTextureFactor2.r = 1.0f;
	ms_activeInventoryTextureFactor2.g = 1.0f;
	ms_activeInventoryTextureFactor2.b = 1.0f;
	ms_activeInventoryTextureFactor2.a = 1.0f;
...
void Direct3d9::setActiveTextureFactorForInventory(VectorRgba const *textureFactors, int count)
{
	if (textureFactors && count > 0)
		ms_activeInventoryTextureFactor = textureFactors[0];
	if (textureFactors && count > 1)
		ms_activeInventoryTextureFactor2 = textureFactors[1];
}
...
bool Direct3d9::install(Gl_install *gl_install)
{
	...
	ms_fogLogCount = 0;
	ms_fogEnabled = false;
	ms_activeFogMode = ShaderImplementation::Pass::FM_Normal;
	setDefaultInventoryMaterialAndFactor();
#ifdef VSPS
...
bool Direct3d9Namespace::present(bool windowed, HWND window, int width, int height)
{
	...
	if (!ms_autoCaptureDone)
	{
		...
		if (length > 0 && length < sizeof(captureBase))
		{
			...
			DWORD const presentLength = GetEnvironmentVariableA("SWG_D3D9_AUTOCAPTURE_PRESENT", capturePresentText, sizeof(capturePresentText));
			if (presentLength > 0 && presentLength < sizeof(capturePresentText))
				capturePresent = atoi(capturePresentText);
			if (capturePresent < 1)
				capturePresent = 1;

			bool const captureByPresent = !isRendererInventoryEnabled() && nextPresentCount >= capturePresent;
			int const captureMax = getRendererAutoCaptureInt("SWG_RENDERER_AUTOCAPTURE_MAX", 1, 1);
			int const captureInterval = getRendererAutoCaptureInt("SWG_RENDERER_AUTOCAPTURE_INTERVAL", 1, 1);
			...
			if ((ms_autoCaptureRequested || captureByPresent) && intervalReady)
			{
				...
				rendererInventoryDiag("d3d9 autocapture %s pre-present=%d capture=%d/%d path=%s.bmp latest=%s.bmp", captured ? "ok" : "failed", nextPresentCount, ms_autoCaptureCount, captureMax, captureSeriesBase, captureBase);
			}
		}
	}
...
void Direct3d9Namespace::setFog(bool enabled, float density, const PackedArgb &color)
{
	...
	if (isRendererInventoryEnabled() && ms_fogLogCount < 512)
	{
		rendererInventoryDiag("fog set d3d9 enabled=%d density=%f color=%u,%u,%u,%u packed=0x%08x count=%d enabledCount=%d",
			enabled ? 1 : 0,
			density,
...
inline void Direct3d9::drawPrimitive(D3DPRIMITIVETYPE primitiveType, int startVertex, int primitiveCount)
{
	...
	if (drawPrimitive())
	{
		...
		logRendererInventoryDraw("draw", primitiveType, 0, startVertex, primitiveCount, 0);
...
inline void Direct3d9::drawIndexedPrimitive(D3DPRIMITIVETYPE primitiveType, int baseIndex, int minimumVertexIndex, int numberOfVertices, int startIndex, int primitiveCount, int numberOfIndices)
{
	...
	if (drawPrimitive())
	{
		...
		logRendererInventoryDraw("drawIndexed", primitiveType, baseIndex, startIndex, primitiveCount, numberOfIndices);
