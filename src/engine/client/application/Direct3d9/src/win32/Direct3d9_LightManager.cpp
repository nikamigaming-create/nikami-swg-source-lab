...
void Direct3d9_LightManager::getInventoryLighting(float ambient[4], float diffuse[4], float direction[4], int &enabledLights)
{
	ambient[0] = ms_currentLights.ambient.r;
	ambient[1] = ms_currentLights.ambient.g;
	ambient[2] = ms_currentLights.ambient.b;
	ambient[3] = ms_currentLights.ambient.a;
	diffuse[0] = 0.0f;
	diffuse[1] = 0.0f;
	diffuse[2] = 0.0f;
	diffuse[3] = 1.0f;
	...
	if (ms_currentLights.ambient.r != 0.0f || ms_currentLights.ambient.g != 0.0f || ms_currentLights.ambient.b != 0.0f)
		enabledLights |= 0x0001;

	const Light *selectedParallelLights[3] =
	{
		ms_currentLights.parallelSpecular[0],
		ms_currentLights.parallel[0],
		ms_currentLights.parallel[1]
	};

	for (int i = 0; i < 3; ++i)
	{
		const Light *light = selectedParallelLights[i];
		if (!light)
			continue;
		...
		if (diffuse[0] == 0.0f && diffuse[1] == 0.0f && diffuse[2] == 0.0f)
		{
			...
			diffuse[3] = diffuseColor.a;

			const Vector &lightDirection = light->getObjectFrameK_w();
			float const length = sqrtf(lightDirection.x * lightDirection.x + lightDirection.y * lightDirection.y + lightDirection.z * lightDirection.z);
			if (length > 0.00001f)
			{
				direction[0] = lightDirection.x / length;
				direction[1] = lightDirection.y / length;
				direction[2] = lightDirection.z / length;
			}
		}
	}

	if (ms_currentLights.pointSpecular[0])
		enabledLights |= 0x0010;
	for (int i = 0; i < PointCount; ++i)
	{
		if (ms_currentLights.point[i])
			enabledLights |= (1 << (i + 5));
	}
}
