...
namespace Direct3d9_VertexShaderDataNamespace
{
	...
	public:
		...
		Include(CrcString const &fileName, int length, byte *data);
		~Include();
	...
	bool readShaderIncludeFile(char const *fileName, int &length, byte *&data);
	...
	m_data(file->readEntireFileAndClose())
{
	if (m_length <= 0 || !m_data)
	{
		...
		m_data = 0;
		m_length = 0;
		readShaderIncludeFile(fileName.getString(), m_length, m_data);
	}
}

// ----------------------------------------------------------------------

Direct3d9_VertexShaderDataNamespace::Include::Include(CrcString const &fileName, int length, byte *data)
:
	m_fileName(fileName),
	m_length(length),
	m_data(data)
	...
	if (Direct3d9::engineOwnsWindow())
	{
		...
		if (i == ms_includeCache->end())
		{
			...
			if (!file)
			{
				int length = 0;
				byte *data = 0;
				if (!readShaderIncludeFile(pFileName, length, data))
				{
					DEBUG_FATAL(true, ("could not open include %s", pFileName));
					return STG_E_FILENOTFOUND;
				}

				Include *include = new Include(findFileName, length, data);
...
bool Direct3d9_VertexShaderDataNamespace::readShaderIncludeFile(char const *fileName, int &length, byte *&data)
{
	length = 0;
	data = 0;
	...
	std::string extractedSlotDefPath("extracted_slot_def/");
	extractedSlotDefPath += fileName;
	candidates[1] = extractedSlotDefPath.c_str();
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		long const fileLength = ftell(file);
		fseek(file, 0, SEEK_SET);

		if (fileLength > 0)
		{
			byte *buffer = new byte[fileLength];
			size_t const bytesRead = fread(buffer, 1, static_cast<size_t>(fileLength), file);
			fclose(file);
