...
{
	typedef void (*SetVrTvModeEnabledFunction)(int enabled);
	...
	void  queueVrUiInput(int x, int y, bool inside, bool leftButtonDown, bool leftButtonUp);
	void  queueVrMouseInput(int x, int y, bool inside, int button, bool buttonDown, bool buttonUp);
	void  queueVrControllerInput(float leftX, float leftY, bool leftActive, float rightX, float rightY, bool rightActive, float headYawRadians, bool headYawValid, float leftHandYawRadians, bool leftHandYawValid, float rightHandYawRadians, bool rightHandYawValid, bool turnModeClickPressed);
	...
	HINSTANCE                                 ms_dll;
	const Gl_api                             *ms_api;
	SetVrTvModeEnabledFunction                ms_setVrTvModeEnabled;
...
bool Graphics::install()
{
	...
	ms_setVrTvModeEnabled = reinterpret_cast<SetVrTvModeEnabledFunction>(GetProcAddress(ms_dll, "SetVrTvModeEnabled"));
	...
	LocalMachineOptionManager::registerOption(ms_vrTvMode, "ClientGraphics", "vrTvMode");
	...
	gl_install.vrTvMode                 = ms_vrTvMode;
	gl_install.windowedModeChanged      = windowedModeChanged;
	gl_install.vrUiInput                = GraphicsNamespace::queueVrUiInput;
	gl_install.vrMouseInput             = GraphicsNamespace::queueVrMouseInput;
	gl_install.vrControllerInput        = GraphicsNamespace::queueVrControllerInput;
	gl_install.vrMenuInput              = GraphicsNamespace::queueVrMenuInput;
...
void Graphics::setVrTvModeEnabled(bool enabled)
{
	if (ms_setVrTvModeEnabled)
		ms_setVrTvModeEnabled(enabled ? 1 : 0);
}
...
bool Graphics::vrIsWorldRenderingEnabled()
{
	return ms_api && ms_api->vrIsWorldRenderingEnabled && ms_api->vrIsWorldRenderingEnabled();
}

// ----------------------------------------------------------------------

bool Graphics::vrBeginWorldFrame()
{
	return ms_api && ms_api->vrBeginWorldFrame && ms_api->vrBeginWorldFrame();
}

// ----------------------------------------------------------------------

bool Graphics::vrGetEyeInfo(int eye, Gl_vrEyeInfo &eyeInfo)
{
	return ms_api && ms_api->vrGetEyeInfo && ms_api->vrGetEyeInfo(eye, &eyeInfo);
}

// ----------------------------------------------------------------------

bool Graphics::vrBeginEye(int eye)
{
	Gl_vrEyeInfo eyeInfo;
	memset(&eyeInfo, 0, sizeof(eyeInfo));
	if (!vrGetEyeInfo(eye, eyeInfo))
		return false;

	if (!ms_api || !ms_api->vrBeginEye || !ms_api->vrBeginEye(eye))
		return false;

	ms_currentRenderTargetWidth = ms_currentRenderTargetMaxWidth = eyeInfo.width;
	ms_currentRenderTargetHeight = ms_currentRenderTargetMaxHeight = eyeInfo.height;
	return true;
}

// ----------------------------------------------------------------------

void Graphics::vrEndEye(int eye)
{
	if (ms_api && ms_api->vrEndEye)
		ms_api->vrEndEye(eye);
...
void Graphics::vrEndWorldFrame()
{
	if (ms_api && ms_api->vrEndWorldFrame)
		ms_api->vrEndWorldFrame();
}

// ----------------------------------------------------------------------

void Graphics::vrEndWorldFrameWithMenuQuad()
{
	if (ms_api && ms_api->vrEndWorldFrameWithMenuQuad)
		ms_api->vrEndWorldFrameWithMenuQuad();
}

// ----------------------------------------------------------------------

bool Graphics::vrBeginHudCapture(int *width, int *height)
{
	int captureWidth = 0;
	int captureHeight = 0;
	if (!ms_api || !ms_api->vrBeginHudCapture || !ms_api->vrBeginHudCapture(&captureWidth, &captureHeight))
		return false;

	if (captureWidth <= 0)
		captureWidth = ms_frameBufferMaxWidth;
	if (captureHeight <= 0)
		captureHeight = ms_frameBufferMaxHeight;
	ms_currentRenderTargetWidth = ms_currentRenderTargetMaxWidth = captureWidth;
	ms_currentRenderTargetHeight = ms_currentRenderTargetMaxHeight = captureHeight;
	if (width)
		*width = captureWidth;
	if (height)
...
bool Graphics::vrBeginHudCapture()
{
	return vrBeginHudCapture(0, 0);
}

// ----------------------------------------------------------------------

void Graphics::vrEndHudCapture()
{
	if (ms_api && ms_api->vrEndHudCapture)
		ms_api->vrEndHudCapture();
...
bool Graphics::vrBeginWristDashboardCapture(int *width, int *height)
{
	int captureWidth = 0;
	int captureHeight = 0;
	if (!ms_api || !ms_api->vrBeginWristDashboardCapture || !ms_api->vrBeginWristDashboardCapture(&captureWidth, &captureHeight))
		return false;
...
void Graphics::vrEndWristDashboardCapture()
{
	if (ms_api && ms_api->vrEndWristDashboardCapture)
		ms_api->vrEndWristDashboardCapture();
...
void Graphics::vrSubmitHudPanelRect(char const *panelName, int left, int top, int right, int bottom, int sourceWidth, int sourceHeight)
{
	if (ms_api && ms_api->vrSubmitHudPanelRect)
		ms_api->vrSubmitHudPanelRect(panelName, left, top, right, bottom, sourceWidth, sourceHeight);
}

// ----------------------------------------------------------------------

void Graphics::vrSubmitHudPanelAnchor(char const *panelName, float x, float y, float z, float radius, bool valid)
{
	if (ms_api && ms_api->vrSubmitHudPanelAnchor)
		ms_api->vrSubmitHudPanelAnchor(panelName, x, y, z, radius, valid);
}

// ----------------------------------------------------------------------

void Graphics::vrSubmitObjectContextInputRegion(int slot, int textureLeft, int textureTop, int textureRight, int textureBottom, int clientLeft, int clientTop, int clientRight, int clientBottom, bool active)
{
	if (ms_api && ms_api->vrSubmitObjectContextInputRegion)
		ms_api->vrSubmitObjectContextInputRegion(slot, textureLeft, textureTop, textureRight, textureBottom, clientLeft, clientTop, clientRight, clientBottom, active);
}

// ----------------------------------------------------------------------

void Graphics::vrSubmitObjectContext(char const *targetName, int health, int healthMax, int action, int actionMax, int mind, int mindMax, bool attackable)
{
	if (ms_api && ms_api->vrSubmitObjectContext)
		ms_api->vrSubmitObjectContext(targetName, health, healthMax, action, actionMax, mind, mindMax, attackable);
}

// ----------------------------------------------------------------------

void Graphics::vrSubmitHoverTargetContext(char const *targetName, int health, int healthMax, int action, int actionMax, int mind, int mindMax, bool attackable)
{
	if (ms_api && ms_api->vrSubmitHoverTargetContext)
		ms_api->vrSubmitHoverTargetContext(targetName, health, healthMax, action, actionMax, mind, mindMax, attackable);
}

// ----------------------------------------------------------------------

void Graphics::vrSubmitWristDashboard(float playerX, float playerZ, float headingRadians, int health, int healthMax, int action, int actionMax, int mind, int mindMax, bool valid)
{
	if (ms_api && ms_api->vrSubmitWristDashboard)
		ms_api->vrSubmitWristDashboard(playerX, playerZ, headingRadians, health, healthMax, action, actionMax, mind, mindMax, valid);
}
...
void GraphicsNamespace::queueVrUiInput(int x, int y, bool inside, bool leftButtonDown, bool leftButtonUp)
{
	queueVrMouseInput(x, y, inside, 0, leftButtonDown, leftButtonUp);
}

// ----------------------------------------------------------------------

void GraphicsNamespace::queueVrMouseInput(int x, int y, bool inside, int button, bool buttonDown, bool buttonUp)
{
	if (!inside)
		return;

	Os::queueSetSystemMouseCursorPosition(x, y);

	if (buttonDown)
		Os::queueMouseButtonDown(0, button);

	if (buttonUp)
		Os::queueMouseButtonUp(0, button);
}

// ----------------------------------------------------------------------

void GraphicsNamespace::queueVrControllerInput(float leftX, float leftY, bool leftActive, float rightX, float rightY, bool rightActive, float headYawRadians, bool headYawValid, float leftHandYawRadians, bool leftHandYawValid, float rightHandYawRadians, bool rightHandYawValid, bool turnModeClickPressed)
{
	Os::queueVrControllerInput(leftX, leftY, leftActive, rightX, rightY, rightActive, headYawRadians, headYawValid, leftHandYawRadians, leftHandYawValid, rightHandYawRadians, rightHandYawValid, turnModeClickPressed);
}

// ----------------------------------------------------------------------

void GraphicsNamespace::queueVrMenuInput(bool pressed)
{
	Os::queueVrMenuInput(pressed);
}
