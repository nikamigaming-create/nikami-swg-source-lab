...
public:
	...
	{
	public:
		VrEyeOverride(Camera &camera, Gl_vrEyeInfo const &eyeInfo);
		~VrEyeOverride();

		bool isActive() const;

	private:
		VrEyeOverride(VrEyeOverride const &);
		VrEyeOverride &operator =(VrEyeOverride const &);

	private:
		Camera *m_camera;
		Transform m_objectToParent;
		GlMatrix4x4 m_projectionMatrix;
		int m_viewportX0;
		int m_viewportY0;
		int m_viewportWidth;
		int m_viewportHeight;
		float m_oneOverViewportWidth;
		float m_oneOverViewportHeight;
		real m_nearPlane;
		Vector m_frustumVertex[FV_Max];
		Plane m_frustumPlane[FP_Max];
