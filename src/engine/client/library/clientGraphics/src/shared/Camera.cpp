...
namespace
{
	Vector rotateOpenXrVectorToCameraLocal(float x, float y, float z, Gl_vrEyeInfo const &eyeInfo)
	{
		float const qx = eyeInfo.orientation[0];
		float const qy = eyeInfo.orientation[1];
		float const qz = eyeInfo.orientation[2];
		float const qw = eyeInfo.orientation[3];

		float const tx = 2.0f * (qy * z - qz * y);
		float const ty = 2.0f * (qz * x - qx * z);
		float const tz = 2.0f * (qx * y - qy * x);

		float const rx = x + qw * tx + (qy * tz - qz * ty);
		float const ry = y + qw * ty + (qz * tx - qx * tz);
		float const rz = z + qw * tz + (qx * ty - qy * tx);

		return Vector(rx, ry, -rz);
	}

	bool buildOpenXrCameraBasis(Gl_vrEyeInfo const &eyeInfo, Vector &i_o, Vector &j_o, Vector &k_o)
	{
		i_o = rotateOpenXrVectorToCameraLocal(1.0f, 0.0f, 0.0f, eyeInfo);
		j_o = rotateOpenXrVectorToCameraLocal(0.0f, 1.0f, 0.0f, eyeInfo);
		k_o = rotateOpenXrVectorToCameraLocal(0.0f, 0.0f, -1.0f, eyeInfo);

		if (!i_o.normalize() || !j_o.normalize() || !k_o.normalize())
			return false;

		return true;
	}

	float clampVrCameraFloat(float value, float minimumValue, float maximumValue)
	{
	...
	float getVrCameraEnvironmentFloat(char const *name, float defaultValue, float minimumValue, float maximumValue)
	{
		char const * const value = getenv(name);
		if (!value || !*value)
			return defaultValue;

		return clampVrCameraFloat(static_cast<float>(atof(value)), minimumValue, maximumValue);
	}
}
...
Camera::VrEyeOverride::VrEyeOverride(Camera &camera, Gl_vrEyeInfo const &eyeInfo)
: m_camera(&camera),
	m_objectToParent(camera.getTransform_o2p()),
	m_projectionMatrix(camera.projectionMatrix),
	m_viewportX0(camera.viewportX0),
	m_viewportY0(camera.viewportY0),
	m_viewportWidth(camera.viewportWidth),
	m_viewportHeight(camera.viewportHeight),
	m_oneOverViewportWidth(camera.oneOverViewportWidth),
	m_oneOverViewportHeight(camera.oneOverViewportHeight),
	m_nearPlane(camera.nearPlane),
	m_projectionMode(camera.projectionMode)
{
	for (int i = 0; i < FV_Max; ++i)
		m_frustumVertex[i] = camera.frustumVertex[i];
	for (int i = 0; i < FP_Max; ++i)
		m_frustumPlane[i] = camera.frustumVolume.getPlane(i);

	if (eyeInfo.width <= 0 || eyeInfo.height <= 0)
		return;

	camera.viewportX0 = 0;
	camera.viewportY0 = 0;
	camera.viewportWidth = eyeInfo.width;
	camera.viewportHeight = eyeInfo.height;
	camera.oneOverViewportWidth = 1.0f / static_cast<float>(eyeInfo.width);
	camera.oneOverViewportHeight = 1.0f / static_cast<float>(eyeInfo.height);
	camera.projectionMode = PM_Perspective;

	real const requestedVrNearZ = static_cast<real>(getVrCameraEnvironmentFloat("SWG_OG_VR_NEAR_PLANE", 0.10f, 0.02f, 1.00f));
	real const nearZ = camera.nearPlane < requestedVrNearZ ? camera.nearPlane : requestedVrNearZ;
	real const farZ = camera.farPlane;
	camera.nearPlane = nearZ;
	real const leftNear = nearZ * tan(eyeInfo.fov[0]);
	real const rightNear = nearZ * tan(eyeInfo.fov[1]);
	real const topNear = nearZ * tan(eyeInfo.fov[2]);
	real const bottomNear = nearZ * tan(eyeInfo.fov[3]);
	real const leftFar = farZ * tan(eyeInfo.fov[0]);
	real const rightFar = farZ * tan(eyeInfo.fov[1]);
	real const topFar = farZ * tan(eyeInfo.fov[2]);
	real const bottomFar = farZ * tan(eyeInfo.fov[3]);

	camera.frustumVertex[FV_NearUpperLeft ].set(leftNear,  topNear,    nearZ);
	camera.frustumVertex[FV_NearUpperRight].set(rightNear, topNear,    nearZ);
	camera.frustumVertex[FV_NearLowerRight].set(rightNear, bottomNear, nearZ);
	camera.frustumVertex[FV_NearLowerLeft ].set(leftNear,  bottomNear, nearZ);
	camera.frustumVertex[FV_FarUpperLeft  ].set(leftFar,   topFar,     farZ);
	camera.frustumVertex[FV_FarUpperRight ].set(rightFar,  topFar,     farZ);
	camera.frustumVertex[FV_FarLowerRight ].set(rightFar,  bottomFar,  farZ);
	camera.frustumVertex[FV_FarLowerLeft  ].set(leftFar,   bottomFar,  farZ);

	camera.frustumVolume.setPlane(FP_Near,   Plane(Vector::negativeUnitZ, nearZ));
	camera.frustumVolume.setPlane(FP_Far,    Plane(Vector::unitZ,        -farZ));
	camera.frustumVolume.setPlane(FP_Top,    Plane(Vector::zero, camera.frustumVertex[4], camera.frustumVertex[5]));
	camera.frustumVolume.setPlane(FP_Right,  Plane(Vector::zero, camera.frustumVertex[5], camera.frustumVertex[6]));
	camera.frustumVolume.setPlane(FP_Bottom, Plane(Vector::zero, camera.frustumVertex[6], camera.frustumVertex[7]));
	camera.frustumVolume.setPlane(FP_Left,   Plane(Vector::zero, camera.frustumVertex[7], camera.frustumVertex[4]));

	real const width = rightNear - leftNear;
	real const height = topNear - bottomNear;
	real const Q = farZ / (farZ - nearZ);

	camera.projectionMatrix.matrix[0][0] = CONST_REAL(2) * nearZ / width;
	camera.projectionMatrix.matrix[0][1] = CONST_REAL(0);
	camera.projectionMatrix.matrix[0][2] = -(rightNear + leftNear) / width;
	camera.projectionMatrix.matrix[0][3] = CONST_REAL(0);

	camera.projectionMatrix.matrix[1][0] = CONST_REAL(0);
	camera.projectionMatrix.matrix[1][1] = CONST_REAL(2) * nearZ / height;
	camera.projectionMatrix.matrix[1][2] = -(topNear + bottomNear) / height;
	camera.projectionMatrix.matrix[1][3] = CONST_REAL(0);

	camera.projectionMatrix.matrix[2][0] = CONST_REAL(0);
	camera.projectionMatrix.matrix[2][1] = CONST_REAL(0);
	camera.projectionMatrix.matrix[2][2] = Q;
	camera.projectionMatrix.matrix[2][3] = -nearZ * Q;

	camera.projectionMatrix.matrix[3][0] = CONST_REAL(0);
	camera.projectionMatrix.matrix[3][1] = CONST_REAL(0);
	camera.projectionMatrix.matrix[3][2] = CONST_REAL(1);
	camera.projectionMatrix.matrix[3][3] = CONST_REAL(0);

	Transform const baseObjectToParent(camera.getTransform_o2p());
	Transform objectToParent(baseObjectToParent);
	Vector headI_o;
	Vector headJ_o;
	Vector headK_o;
	if (buildOpenXrCameraBasis(eyeInfo, headI_o, headJ_o, headK_o))
	{
		Vector const headI_p(objectToParent.rotate_l2p(headI_o));
		Vector const headJ_p(objectToParent.rotate_l2p(headJ_o));
		Vector const headK_p(objectToParent.rotate_l2p(headK_o));
		objectToParent.setLocalFrameIJK_p(headI_p, headJ_p, headK_p);
	}

	Vector const eyeOffset_o(eyeInfo.position[0], eyeInfo.position[1], -eyeInfo.position[2]);
	objectToParent.move_p(baseObjectToParent.rotate_l2p(eyeOffset_o));
	camera.setTransform_o2p(objectToParent);
	camera._updateWorldFrustumVolume();
}

// ----------------------------------------------------------------------

Camera::VrEyeOverride::~VrEyeOverride()
{
	if (!m_camera)
		return;

	m_camera->projectionMatrix = m_projectionMatrix;
	m_camera->viewportX0 = m_viewportX0;
	m_camera->viewportY0 = m_viewportY0;
	m_camera->viewportWidth = m_viewportWidth;
	m_camera->viewportHeight = m_viewportHeight;
	m_camera->oneOverViewportWidth = m_oneOverViewportWidth;
	m_camera->oneOverViewportHeight = m_oneOverViewportHeight;
	m_camera->nearPlane = m_nearPlane;
	m_camera->projectionMode = m_projectionMode;
	for (int i = 0; i < FV_Max; ++i)
		m_camera->frustumVertex[i] = m_frustumVertex[i];
	for (int i = 0; i < FP_Max; ++i)
		m_camera->frustumVolume.setPlane(i, m_frustumPlane[i]);
	m_camera->setTransform_o2p(m_objectToParent);
	m_camera->_updateWorldFrustumVolume();
	m_camera = 0;
}

// ----------------------------------------------------------------------

bool Camera::VrEyeOverride::isActive() const
{
	return m_camera != 0;
}
