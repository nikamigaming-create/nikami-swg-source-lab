...
: AppearanceTemplate(fileName),
	...
	m_shadowVolume(NULL),
	m_shadowVolumeStreamerChecked(false),
	m_shadowVolumeAllowed(true),
	...
	if (couldRenderShadow)
	{
		...
		if (object)
		{
			...
			if (shouldRenderShadow)
			{
				if (!m_shadowVolume)
				{
					if (!m_shadowVolumeStreamerChecked && !ms_disableShadowVolumeStreamerDetector)
					{
						for (int i = 0; i < m_shaderPrimitiveSetTemplate->getNumberOfShaderPrimitiveTemplates(); i++)
						{
							ShaderPrimitiveSetTemplate::LocalShaderPrimitiveTemplate const & shaderPrimitiveTemplate = m_shaderPrimitiveSetTemplate->getShaderPrimitiveTemplate(i);
							...
							{
								ShadowVolumeStreamerDetector detector (getName ());
								detector.addPrimitive (*shaderPrimitiveTemplate.getVertexBuffer(), *shaderPrimitiveTemplate.getIndexBuffer());
								if (detector.detectAndReport ())
								{
									m_shadowVolumeAllowed = false;
									break;
								}
							}
						}

						m_shadowVolumeStreamerChecked = true;
					}

					if (m_shadowVolumeAllowed)
					{
