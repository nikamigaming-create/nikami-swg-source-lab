...
namespace CuiIoWinNamespace
{
	...
	int s_vrSnapTurnHeldDirection = 0;
	float s_vrSnapTurnNextTime = 0.0f;
	bool s_vrTurnModeSmooth = false;
	bool s_vrTurnModeInitialized = false;
	bool s_vrTurnModeClickDown = false;
	float s_vrSmoothedMoveX = 0.0f;
	float s_vrSmoothedMoveY = 0.0f;
	float s_vrLastMoveUpdateTime = 0.0f;

	struct VrStickVector
	{
		float x;
		float y;
	};

	float clampVrFloat(float value, float minimumValue, float maximumValue)
	{
	...
	float getVrEnvironmentFloat(char const *name, float defaultValue, float minimumValue, float maximumValue)
	{
		char const * const value = getenv(name);
		if (!value || !*value)
			return defaultValue;

		return clampVrFloat(static_cast<float>(atof(value)), minimumValue, maximumValue);
	}

	bool getVrEnvironmentEquals(char const *name, char const *expectedValue)
	{
		char const * const value = getenv(name);
		return equalsIgnoreCase(value, expectedValue);
	}

	bool getDefaultVrTurnModeSmooth()
	{
		return getVrEnvironmentEquals("SWG_OG_VR_TURN_MODE", "smooth") ||
			getVrEnvironmentEquals("SWG_OG_VR_TURN_MODE", "continuous");
	}

	char const *getVrTracePath()
	{
		char const * tracePath = getenv("SWG_OG_VR_MOVEMENT_TRACE_FILE");
		if (!tracePath || !*tracePath)
			tracePath = getenv("SWG_OG_VR_PROOF");
		return tracePath && *tracePath ? tracePath : 0;
	}

	VrStickVector applyVrRadialStickShape(float x, float y, bool active, float deadZone, float curve)
	{
		VrStickVector result = { 0.0f, 0.0f };
		if (!active)
			return result;

		x = clampVrFloat(x, -1.0f, 1.0f);
		y = clampVrFloat(y, -1.0f, 1.0f);
		float const magnitude = sqrtf(x * x + y * y);
		if (magnitude <= deadZone || magnitude <= 0.0001f)
			return result;

		float normalized = (magnitude - deadZone) / (1.0f - deadZone);
		normalized = clampVrFloat(normalized, 0.0f, 1.0f);
		float shapedMagnitude = normalized;
		if (curve > 0.01f && fabsf(curve - 1.0f) > 0.001f)
			shapedMagnitude = powf(normalized, curve);

		float const unitScale = shapedMagnitude / magnitude;
		result.x = x * unitScale;
		result.y = y * unitScale;
		return result;
	}

	float applyVrTurnAxisShape(float value)
	{
		float const deadZone = getVrEnvironmentFloat("SWG_OG_VR_TURN_DEADZONE", 0.35f, 0.01f, 0.95f);
		float const curve = getVrEnvironmentFloat("SWG_OG_VR_TURN_CURVE", 1.00f, 0.25f, 4.00f);
		float const magnitude = fabsf(value);
		if (magnitude <= deadZone)
			return 0.0f;

		float const normalized = clampVrFloat((magnitude - deadZone) / (1.0f - deadZone), 0.0f, 1.0f);
		float const shaped = curve > 0.01f && fabsf(curve - 1.0f) > 0.001f ? powf(normalized, curve) : normalized;
		return value < 0.0f ? -shaped : shaped;
	}

	void smoothVrStickVector(VrStickVector const &target, float elapsedTime)
	{
		float const accelerationPerSecond = getVrEnvironmentFloat("SWG_OG_VR_MOVE_ACCEL_PER_SECOND", 20.0f, 0.5f, 120.0f);
		float const decelerationPerSecond = getVrEnvironmentFloat("SWG_OG_VR_MOVE_DECEL_PER_SECOND", 30.0f, 0.5f, 160.0f);
		float const currentMagnitude = sqrtf(s_vrSmoothedMoveX * s_vrSmoothedMoveX + s_vrSmoothedMoveY * s_vrSmoothedMoveY);
		float const targetMagnitude = sqrtf(target.x * target.x + target.y * target.y);
		float const dot = s_vrSmoothedMoveX * target.x + s_vrSmoothedMoveY * target.y;
		bool const decelerating = targetMagnitude <= 0.0001f || targetMagnitude < currentMagnitude || dot < 0.0f;
		float const maxDelta = (decelerating ? decelerationPerSecond : accelerationPerSecond) * elapsedTime;
		float const deltaX = target.x - s_vrSmoothedMoveX;
		float const deltaY = target.y - s_vrSmoothedMoveY;
		float const deltaMagnitude = sqrtf(deltaX * deltaX + deltaY * deltaY);
		if (deltaMagnitude <= maxDelta || deltaMagnitude <= 0.0001f)
		{
			s_vrSmoothedMoveX = target.x;
			s_vrSmoothedMoveY = target.y;
			return;
		}

		float const scale = maxDelta / deltaMagnitude;
		s_vrSmoothedMoveX += deltaX * scale;
		s_vrSmoothedMoveY += deltaY * scale;
	}

	void rotateVrMovementReference(VrStickVector &move, float referenceYawRadians)
	{
		float const cosYaw = cosf(referenceYawRadians);
		float const sinYaw = sinf(referenceYawRadians);
		float const rotatedX = move.x * cosYaw + move.y * sinYaw;
		float const rotatedY = -move.x * sinYaw + move.y * cosYaw;
		move.x = rotatedX;
		move.y = rotatedY;
	}

	void executeVrMovementMessage(InputMap &inputMap, int message, float value)
	{
		MessageQueue * const messageQueue = inputMap.getMessageQueue();
		if (!messageQueue)
			return;

		messageQueue->appendMessage(message, value);
	}

	void executeVrSnapTurn(InputMap &inputMap, int direction)
	{
		MessageQueue * const messageQueue = inputMap.getMessageQueue();
		if (!messageQueue)
			return;

		float const snapDegrees = getVrEnvironmentFloat("SWG_OG_VR_SNAP_TURN_DEGREES", 30.0f, 5.0f, 90.0f);
		float const snapYawRadians = snapDegrees * 0.01745329252f;
		messageQueue->appendMessage(CM_cameraYawMouse, direction > 0 ? snapYawRadians : -snapYawRadians);
	}

	void executeVrSmoothTurn(InputMap &inputMap, float turnX, float elapsedTime)
	{
		MessageQueue * const messageQueue = inputMap.getMessageQueue();
		if (!messageQueue)
			return;

		float const degreesPerSecond = getVrEnvironmentFloat("SWG_OG_VR_SMOOTH_TURN_DEGREES_PER_SECOND", 90.0f, 15.0f, 360.0f);
		float const yawRadians = turnX * degreesPerSecond * 0.01745329252f * elapsedTime;
		if (yawRadians != 0.0f)
			messageQueue->appendMessage(CM_cameraYawMouse, yawRadians);
	}

	void traceVrTurnMode(char const *mode, char const *reason)
	{
		char const * const tracePath = getVrTracePath();
		if (!tracePath)
			return;
		...
		fprintf(file, "{\"event\":\"ogVrTurnMode\",\"mode\":\"%s\",\"reason\":\"%s\"}\n",
			mode ? mode : "",
			reason ? reason : "");
		fclose(file);
	}

	void traceVrSnapTurn(int direction, float snapDegrees, char const *turnMode)
	{
		char const * const tracePath = getVrTracePath();
		if (!tracePath)
			return;
		...
		fprintf(file, "{\"event\":\"ogVrSnapTurn\",\"direction\":%d,\"snapDegrees\":%0.3f,\"turnMode\":\"%s\"}\n",
			direction,
			snapDegrees,
			turnMode ? turnMode : "");
		fclose(file);
	...
	void traceVrMovement(float rawLeftX, float rawLeftY, VrStickVector const &targetMove, char const *referenceMode, bool referenceYawValid, float rightX, char const *turnMode, bool turnModeClickPressed)
	{
		char const * const tracePath = getVrTracePath();
		if (!tracePath)
			return;

		static int s_vrMovementTraceCount = 0;
		++s_vrMovementTraceCount;
		if (s_vrMovementTraceCount > 160 && (s_vrMovementTraceCount % 120) != 0)
			return;
		...
		fprintf(file, "{\"event\":\"ogVrMovement\",\"rawLeft\":[%0.4f,%0.4f],\"target\":[%0.4f,%0.4f],\"smoothed\":[%0.4f,%0.4f],\"reference\":\"%s\",\"referenceYawValid\":%s,\"rightX\":%0.4f,\"turnMode\":\"%s\",\"turnModeClick\":%s}\n",
			rawLeftX,
			rawLeftY,
			targetMove.x,
			targetMove.y,
	...
	void queueVrControllerInput(float leftX, float leftY, bool leftActive, float rightX, float, bool rightActive, float headYawRadians, bool headYawValid, float leftHandYawRadians, bool leftHandYawValid, float rightHandYawRadians, bool rightHandYawValid, bool turnModeClickPressed)
	{
		InputMap * const inputMap = Game::getGameInputMap();
		if (!inputMap)
		{
			s_vrSmoothedMoveX = 0.0f;
			s_vrSmoothedMoveY = 0.0f;
			s_vrSnapTurnHeldDirection = 0;
			s_vrSnapTurnNextTime = 0.0f;
			s_vrLastMoveUpdateTime = 0.0f;
			s_vrTurnModeClickDown = false;
			return;
		}

		float const now = Game::getElapsedTime();
		float elapsedTime = s_vrLastMoveUpdateTime > 0.0f ? now - s_vrLastMoveUpdateTime : 0.016f;
		s_vrLastMoveUpdateTime = now;
		if (elapsedTime < 0.0f || elapsedTime > 0.100f)
			elapsedTime = 0.016f;

		float const moveDeadZone = getVrEnvironmentFloat("SWG_OG_VR_MOVE_DEADZONE", 0.18f, 0.01f, 0.95f);
		float const moveCurve = getVrEnvironmentFloat("SWG_OG_VR_MOVE_CURVE", 1.00f, 0.25f, 4.00f);
		VrStickVector targetMove = applyVrRadialStickShape(leftX, leftY, leftActive, moveDeadZone, moveCurve);

		char const *referenceMode = "head";
		bool referenceYawValid = headYawValid;
		float referenceYawRadians = headYawRadians;

		if (getVrEnvironmentEquals("SWG_OG_VR_MOVE_REFERENCE", "hand") || getVrEnvironmentEquals("SWG_OG_VR_MOVE_REFERENCE", "left") || getVrEnvironmentEquals("SWG_OG_VR_MOVE_REFERENCE", "left_hand"))
		{
			referenceMode = "left_hand";
			referenceYawRadians = leftHandYawRadians;
			referenceYawValid = leftHandYawValid;
		}
		else if (getVrEnvironmentEquals("SWG_OG_VR_MOVE_REFERENCE", "right") || getVrEnvironmentEquals("SWG_OG_VR_MOVE_REFERENCE", "right_hand"))
		{
			referenceMode = "right_hand";
			referenceYawRadians = rightHandYawRadians;
			referenceYawValid = rightHandYawValid;
		}

		if (referenceYawValid)
			rotateVrMovementReference(targetMove, referenceYawRadians);
		...
		{
			smoothVrStickVector(targetMove, elapsedTime);
		}

		if (fabsf(s_vrSmoothedMoveY) > 0.001f)
			executeVrMovementMessage(*inputMap, static_cast<int>(CM_moveLongitudinal), s_vrSmoothedMoveY);

		if (fabsf(s_vrSmoothedMoveX) > 0.001f)
			executeVrMovementMessage(*inputMap, static_cast<int>(CM_moveLateral), s_vrSmoothedMoveX);

		if (!s_vrTurnModeInitialized)
		{
			s_vrTurnModeSmooth = getDefaultVrTurnModeSmooth();
			s_vrTurnModeInitialized = true;
			traceVrTurnMode(s_vrTurnModeSmooth ? "smooth" : "snap", "initial");
		}

		if (turnModeClickPressed && !s_vrTurnModeClickDown)
		{
			s_vrTurnModeSmooth = !s_vrTurnModeSmooth;
			s_vrSnapTurnHeldDirection = 0;
			s_vrSnapTurnNextTime = 0.0f;
			traceVrTurnMode(s_vrTurnModeSmooth ? "smooth" : "snap", "right-stick-click");
		}
		s_vrTurnModeClickDown = turnModeClickPressed;

		char const * const turnMode = s_vrTurnModeSmooth ? "smooth" : "snap";
		float const turnX = rightActive ? applyVrTurnAxisShape(rightX) : 0.0f;
		if (equalsIgnoreCase(turnMode, "smooth"))
		{
			s_vrSnapTurnHeldDirection = 0;
			s_vrSnapTurnNextTime = 0.0f;
			executeVrSmoothTurn(*inputMap, turnX, elapsedTime);
			traceVrMovement(leftX, leftY, targetMove, referenceMode, referenceYawValid, rightX, turnMode, turnModeClickPressed);
			return;
		}
		...
		{
			int const direction = turnX > 0.0f ? 1 : -1;
			float const now = Game::getElapsedTime();

			if (direction != s_vrSnapTurnHeldDirection || now >= s_vrSnapTurnNextTime)
			{
				executeVrSnapTurn(*inputMap, direction);
				traceVrSnapTurn(direction, getVrEnvironmentFloat("SWG_OG_VR_SNAP_TURN_DEGREES", 30.0f, 5.0f, 90.0f), turnMode);
				s_vrSnapTurnHeldDirection = direction;
				s_vrSnapTurnNextTime = now + getVrEnvironmentFloat("SWG_OG_VR_SNAP_TURN_REPEAT_SECONDS", 0.35f, 0.10f, 2.00f);
			}
		}
	...
	void queueVrMenuInput(bool pressed)
	{
		static bool s_vrMenuPressed = false;
		if (pressed == s_vrMenuPressed)
			return;

		s_vrMenuPressed = pressed;
		if (!pressed)
			return;

		static std::string const s_toggleButtonBarAction("toggleButtonBar");
		if (!CuiActionManager::performAction(s_toggleButtonBarAction, Unicode::emptyString))
			IGNORE_RETURN(CuiActionManager::performAction(CuiActions::gameMenuActivate, Unicode::emptyString));
	}
}
...
m_ignoreNextNumPadValue         (false)
{
	...
	Os::setQueueVrControllerInputHookFunction(queueVrControllerInput);
	Os::setQueueVrMenuInputHookFunction(queueVrMenuInput);
}
...
CuiIoWin::~CuiIoWin ()
{
	Os::setQueueVrControllerInputHookFunction(0);
	Os::setQueueVrMenuInputHookFunction(0);
