...
namespace ClientWeaponObjectTemplateNamespace
{
	...
	typedef stdvector<std::pair<std::string, int> >::fwd String2IntVector;

	DataTable     *ms_weaponEffectDataTable;
	String2IntVector *ms_weaponEffectDataTableIndexMap;
	...
	void writeStartupTrace(char const *stage)
	{
		...
		fprintf(file, "ClientWeaponObjectTemplate:%s\n", stage);
		fclose(file);
	}
}
...
void ClientWeaponObjectTemplate::install(bool allowDefaultTemplateParams)
{
	...
	writeStartupTrace("after-shared-weapon-object-template");
	IGNORE_RETURN(ObjectTemplateList::assignBinding(SharedWeaponObjectTemplate_tag, create));
	writeStartupTrace("after-assign-binding");

	// Create the data table
	delete ms_weaponEffectDataTableIndexMap;
	ms_weaponEffectDataTableIndexMap = new String2IntVector;
	NOT_NULL(ms_weaponEffectDataTableIndexMap);
	writeStartupTrace("after-index-map-create");

	Iff dataTableIff;

	if (dataTableIff.open("datatables/weapon/weapon.iff", true))
	{
		writeStartupTrace("after-weapon-iff-open");
		delete ms_weaponEffectDataTable;
		ms_weaponEffectDataTable = new DataTable();
		NOT_NULL(ms_weaponEffectDataTable);
		writeStartupTrace("after-data-table-create");

		ms_weaponEffectDataTable->load(dataTableIff);
		writeStartupTrace("after-data-table-load");

		// Store the row appearance name and its row index for fast indexing
		// into the DataTable.

		const int rowCount = ms_weaponEffectDataTable->getNumRows();
		writeStartupTrace("before-index-map-build");

		for (int row = 0; row < rowCount; ++row)
		{
			char traceBuffer[128];
			IGNORE_RETURN(snprintf(traceBuffer, sizeof(traceBuffer), "row-%d-before-string", row));
			writeStartupTrace(traceBuffer);
			std::string const &weaponEffect = ms_weaponEffectDataTable->getStringValue(0, row);
			IGNORE_RETURN(snprintf(traceBuffer, sizeof(traceBuffer), "row-%d-after-string", row));
			writeStartupTrace(traceBuffer);
			int const weaponEffectIndex = ms_weaponEffectDataTable->getIntValue(1, row);
			IGNORE_RETURN(snprintf(traceBuffer, sizeof(traceBuffer), "row-%d-after-int", row));
			writeStartupTrace(traceBuffer);

			char weaponIndexString[256];
			IGNORE_RETURN(_itoa(weaponEffectIndex, weaponIndexString, 10));
			IGNORE_RETURN(snprintf(traceBuffer, sizeof(traceBuffer), "row-%d-after-itoa", row));
			writeStartupTrace(traceBuffer);

			std::string lookUpId(weaponEffect + weaponIndexString);
			IGNORE_RETURN(snprintf(traceBuffer, sizeof(traceBuffer), "row-%d-after-lookup-id", row));
			writeStartupTrace(traceBuffer);

			ms_weaponEffectDataTableIndexMap->push_back(std::make_pair(lookUpId, row));
			IGNORE_RETURN(snprintf(traceBuffer, sizeof(traceBuffer), "row-%d-after-push", row));
			writeStartupTrace(traceBuffer);
		}
		writeStartupTrace("after-index-map-build");
	}
	else
		writeStartupTrace("weapon-iff-open-failed");
...
void ClientWeaponObjectTemplate::postLoad ()
{
	...
	int row = -1;
	for (String2IntVector::const_iterator iter = ms_weaponEffectDataTableIndexMap->begin(); iter != ms_weaponEffectDataTableIndexMap->end(); ++iter)
	{
		if (iter->first == m_lookUpId)
		{
			row = iter->second;
			break;
		}
