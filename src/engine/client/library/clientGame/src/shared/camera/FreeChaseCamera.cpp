...
namespace
{
	...
	bool  ms_vrFirstPersonLogged = false;
	bool  ms_vrPitchNeutralizedLogged = false;
	...
	bool getEnvironmentFlag(char const * const name)
	{
		char const * const value = getenv(name);
		if (!value || !*value)
			return false;

		return strcmp(value, "0") != 0 && _stricmp(value, "false") != 0 && _stricmp(value, "off") != 0 && _stricmp(value, "no") != 0;
	}

	float getEnvironmentFloat(char const * const name, float defaultValue)
	{
		char const * const value = getenv(name);
		if (!value || !*value)
			return defaultValue;
	...
	bool isVrFirstPersonForced()
	{
		if (!getEnvironmentFlag("SWG_OG_VR") && !getEnvironmentFlag("SWG_D3D11_VR"))
			return false;

		char const * const overrideValue = getenv("SWG_OG_VR_FORCE_FIRST_PERSON");
		if (overrideValue && *overrideValue)
			return getEnvironmentFlag("SWG_OG_VR_FORCE_FIRST_PERSON");

		return true;
	}

	void appendOgVrTraceLine(char const * const text)
	{
		if (!text || !*text)
			return;

		char const * path = getenv("SWG_OG_VR_HAND_TRACE_FILE");
		if (!path || !*path)
			path = getenv("SWG_OG_VR_PHYSICS_TRACE_FILE");
		if (!path || !*path)
			path = getenv("SWG_OG_VR_PROOF");
		if (!path || !*path)
			return;
	...
	void logVrFirstPersonForce(int previousSetting, int newSetting, float previousZoom, float newZoom, float firstPersonDistance)
	{
		char line[256];
		_snprintf(line, sizeof(line) - 1, "{\"event\":\"ogVrFirstPersonForce\",\"previousSetting\":%d,\"newSetting\":%d,\"previousZoom\":%0.3f,\"newZoom\":%0.3f,\"firstPersonDistance\":%0.3f}", previousSetting, newSetting, previousZoom, newZoom, firstPersonDistance);
		line[sizeof(line) - 1] = '\0';
		appendOgVrTraceLine(line);
	}

	void logVrPitchNeutralized(float previousPitch, float ignoredPitchMod)
	{
		char line[256];
		_snprintf(line, sizeof(line) - 1, "{\"event\":\"ogVrPitchNeutralized\",\"previousPitchRadians\":%0.6f,\"ignoredPitchModRadians\":%0.6f}", previousPitch, ignoredPitchMod);
		line[sizeof(line) - 1] = '\0';
		appendOgVrTraceLine(line);
	}

	void restoreTargetMeshWhenLeavingFirstPerson(Object const * const target)
	{
		if (!target)
			return;

		ClientObject const * const clientTarget = target->asClientObject ();
		...
		{
			SkeletalAppearance2 *const appearance = const_cast<SkeletalAppearance2 *> (target->getAppearance () ? target->getAppearance ()->asSkeletalAppearance2 () : 0);
			if (appearance)
...
float FreeChaseCamera::alter (float elapsedTime)
{
	...
	bool const vrFirstPersonForced = isVrFirstPersonForced();
	...
	if (vrFirstPersonForced)
	{
		float const previousPitch = ms_pitch;
		if (!ms_vrPitchNeutralizedLogged && (previousPitch != 0.0f || pitchMod != 0.0f))
		{
			logVrPitchNeutralized(previousPitch, pitchMod);
			ms_vrPitchNeutralizedLogged = true;
		}
		ms_pitch = 0.0f;
	}
	else
	...
	if (vrFirstPersonForced && m_numberOfSettings > 0 && m_settings)
	{
		int const previousSetting = ms_currentSetting;
		float const previousZoom = m_currentZoom;
		ms_currentSetting = 0;
		m_zoom = 0.0f;
		m_currentZoom = m_zoom;
		if (!ms_vrFirstPersonLogged || previousSetting != ms_currentSetting)
		{
			logVrFirstPersonForce(previousSetting, ms_currentSetting, previousZoom, m_zoom, m_firstPersonDistance);
			ms_vrFirstPersonLogged = true;
		}
	}
	...
	//-- move camera to the avatar's cell and position
	{
		...
		if (vrFirstPersonForced)
		{
			float const eyeForward = getEnvironmentFloat("SWG_OG_VR_FIRST_PERSON_EYE_FORWARD", getEnvironmentFloat("SWG_OG_VR_FIRST_PERSON_HEAD_CLEARANCE", 0.34f));
			float const eyeUp = getEnvironmentFloat("SWG_OG_VR_FIRST_PERSON_EYE_UP", 0.12f);
			move_o(Vector(0.0f, eyeUp, eyeForward));
		}
	}
...
void FreeChaseCamera::setFirstPerson (bool const firstPerson)
{
	...
	{
		m_firstPerson = true;
		bool const keepTargetVisibleForOgVr =
			(getEnvironmentFlag("SWG_OG_VR") || getEnvironmentFlag("SWG_D3D11_VR")) &&
			getEnvironmentFlag("SWG_OG_VR_SHOW_PLAYER_BODY");
		...
		else if (m_target && !keepTargetVisibleForOgVr)
		{
	...
	else
	{
		...
		if (!ConfigClientGame::getShowAttachmentsInFirstPerson())
		{
			...
			restoreTargetMeshWhenLeavingFirstPerson(m_target);
		}
		else if (m_target)
		{
