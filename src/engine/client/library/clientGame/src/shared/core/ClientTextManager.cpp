...
namespace ClientTextManagerNamespace
{
	...
	typedef std::vector<std::pair<Unicode::String, ColorCodedText> >              ColoredTextList;
	typedef std::vector<std::pair<ClientTextManager::TextType, Unicode::String> > DefaultGameTextColors;
	typedef std::vector<std::pair<Unicode::String, std::string> >                 DefaultGameTextColorStringIds;
	...
	void writeClientTextStartupTrace(char const *stage)
	{
		...
		if (!file)
			return;
		fprintf(file, "ClientTextManager:%s\n", stage);
		fclose(file);
	}
	...
	ColoredTextList::iterator findColoredText(ColoredTextList &coloredTextList, Unicode::String const &text);
	ColoredTextList::const_iterator findColoredText(ColoredTextList const &coloredTextList, Unicode::String const &text);
	DefaultGameTextColors::const_iterator findDefaultGameTextColor(ClientTextManager::TextType textType);
}
...
{
	writeClientTextStartupTrace("default-color-entry");
	// Find the row we are interested in

	for (int row = 0; row < dataTable.getNumRows(); ++row)
	{
		writeClientTextStartupTrace("default-color-before-row-string");
		std::string const &rowString = dataTable.getStringValue("stringId", row);
		writeClientTextStartupTrace("default-color-after-row-string");
		StringId stringId("ui_text_color", rowString);

		if (rowString == text)
		{
			writeClientTextStartupTrace("default-color-match");
			...
			writeClientTextStartupTrace("default-color-before-set-game-text");
			ClientTextManager::setColoredGameText(stringId.localize(), packedRgb);
			writeClientTextStartupTrace("default-color-after-set-game-text");

			Unicode::String coloredGameTextName = Unicode::toLower(stringId.localize());
			writeClientTextStartupTrace("default-color-before-default-push");
			s_defaultGameTextColors.push_back(std::make_pair(textType, coloredGameTextName));
			writeClientTextStartupTrace("default-color-after-default-push");
			writeClientTextStartupTrace("default-color-before-string-id-push");
			s_defaultGameTextColorStringIds.push_back(std::make_pair(coloredGameTextName, rowString));
			writeClientTextStartupTrace("default-color-after-string-id-push");

			break;
		}
	}
	writeClientTextStartupTrace("default-color-exit");
}

// ----------------------------------------------------------------------------
ColoredTextList::iterator ClientTextManagerNamespace::findColoredText(ColoredTextList &coloredTextList, Unicode::String const &text)
{
	for (ColoredTextList::iterator iter = coloredTextList.begin(); iter != coloredTextList.end(); ++iter)
	{
		if (iter->first == text)
			return iter;
	}

	return coloredTextList.end();
}

// ----------------------------------------------------------------------------
ColoredTextList::const_iterator ClientTextManagerNamespace::findColoredText(ColoredTextList const &coloredTextList, Unicode::String const &text)
{
	for (ColoredTextList::const_iterator iter = coloredTextList.begin(); iter != coloredTextList.end(); ++iter)
	{
		if (iter->first == text)
			return iter;
	}
...
// ----------------------------------------------------------------------------
DefaultGameTextColors::const_iterator ClientTextManagerNamespace::findDefaultGameTextColor(ClientTextManager::TextType const textType)
{
	for (DefaultGameTextColors::const_iterator iter = s_defaultGameTextColors.begin(); iter != s_defaultGameTextColors.end(); ++iter)
	{
		if (iter->first == textType)
			return iter;
	}

	return s_defaultGameTextColors.end();
}
...
bool ClientTextManagerNamespace::setColoredText(ColoredTextList &coloredTextList, Unicode::String const &text, std::string const &colorCodeText, bool addColor = true)
{
	...
	ColoredTextList::iterator iterColoredTextList = findColoredText(coloredTextList, lowerText);
	...
	else if (addColor)
	{
		...
		coloredTextList.push_back(std::make_pair(lowerText, colorCodedText));
...
{
	ColoredTextList::iterator iterColoredTextList = findColoredText(coloredTextList, Unicode::toLower(text));
...
{
	writeClientTextStartupTrace("install-entry");
	...
	writeClientTextStartupTrace("install-after-exit-chain");

	s_resetColorCode = Unicode::narrowToWide("\\#.");
	writeClientTextStartupTrace("install-after-reset-code");

	// Create a white color code

	ClientTextManagerNamespace::getColorCode(PackedRgb::solidWhite, s_whiteColorCode);
	writeClientTextStartupTrace("install-after-white-code");

	resetColoredGameText();
	writeClientTextStartupTrace("install-exit");
}
...
Unicode::String ClientTextManager::getColorCode(TextType const textType)
{
	...
	DefaultGameTextColors::const_iterator iterDefaultGameTextColors = findDefaultGameTextColor(textType);

	if (iterDefaultGameTextColors != s_defaultGameTextColors.end())
	{
		Unicode::String const &name = iterDefaultGameTextColors->second;

		ColoredTextList::const_iterator iterColoredGameText = findColoredText(s_coloredGameText, name);
...
{
	writeClientTextStartupTrace("reset-entry");
	// Load the default colors

	char const textColorsFile[] = "datatables/chat/default_text_colors.iff";
	Iff textColorsIff;

	writeClientTextStartupTrace("reset-before-open");
	if (textColorsIff.open(textColorsFile, true))
	{
		writeClientTextStartupTrace("reset-after-open-true");
		DataTable * const dataTable = new DataTable;

		writeClientTextStartupTrace("reset-before-load");
		dataTable->load(textColorsIff);
		writeClientTextStartupTrace("reset-after-load");

		writeClientTextStartupTrace("reset-before-clear-defaults");
		s_defaultGameTextColors.clear();
		writeClientTextStartupTrace("reset-after-clear-defaults");

		getDefaultTextColorFromDataTable("chat_channel", TT_chatChannel, *dataTable);
		writeClientTextStartupTrace("reset-after-chat-channel");
		...
		writeClientTextStartupTrace("reset-after-chat-colors");
		...
		writeClientTextStartupTrace("reset-after-combat-colors");
		...
		writeClientTextStartupTrace("reset-after-all-colors");
	}
	else
	{
		writeClientTextStartupTrace("reset-after-open-false");
		DEBUG_WARNING(true, ("Unable to load the default text color data table: %s", textColorsFile));
	}
	writeClientTextStartupTrace("reset-exit");
}
// ============================================================================
