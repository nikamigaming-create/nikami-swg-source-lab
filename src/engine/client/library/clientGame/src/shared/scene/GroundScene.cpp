...
void GroundScene::init (const char* const terrainFilename, CreatureObject* const player, const float timeInSeconds)
{
	...
	Graphics::setVrTvModeEnabled(false);
	Game::_setScene(this);
...
GroundScene::~GroundScene (void)
{
	...
	VrDetachedHands::reset();
	Graphics::setVrTvModeEnabled(true);
...
void GroundScene::draw (void) const
{
	...
	bool renderedVrWorld = false;
	if (Graphics::vrIsWorldRenderingEnabled() && Graphics::vrBeginWorldFrame())
	{
		bool renderedVrEye = false;
		for (int eye = 0; eye < 2; ++eye)
		{
			Gl_vrEyeInfo eyeInfo;
			if (!Graphics::vrGetEyeInfo(eye, eyeInfo) || !Graphics::vrBeginEye(eye))
				continue;

			renderedVrEye = true;
			GameCamera * const camera = const_cast<GameCamera *>(getCurrentCamera());
			Camera::VrEyeOverride const eyeOverride(*camera, eyeInfo);

			Graphics::clearViewport(true, backgroundColor.asUint32(), true, 1.0f, true, 0);

			bool const useLegacyVrShadowAlpha = ConfigFile::getKeyBool("Direct3d11", "legacyStencilShadowsInVr", false);

			//-- legacy alpha/stencil shadows are disabled in VR unless explicitly requested.
			ShadowVolume::setAllowShadowSubmissions(useLegacyVrShadowAlpha);
			...
			if (useLegacyVrShadowAlpha)
				ShadowVolume::renderShadowAlpha(getCurrentCamera());

			Graphics::vrEndEye(eye);
		}
		Graphics::vrEndWorldFrame();
		renderedVrWorld = renderedVrEye;
	}

	bool const renderFlatWorld = !renderedVrWorld;
	if (renderFlatWorld)
	{
