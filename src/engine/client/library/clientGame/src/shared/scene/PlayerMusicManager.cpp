...
namespace PlayerMusicManagerNamespace
{
	...
	struct MusicParticleSystemPathEntry
	{
		PlayerMusicManager::ParticleType type;
		char path[256];
	};

	struct MusicParticleSystemPaths
	{
		MusicParticleSystemPathEntry entries[16];
		int count;
	};

	struct FlourishParticleSystemPathEntry
	{
		int index;
		char path[256];
	};

	struct FlourishParticleSystemPaths
	{
		FlourishParticleSystemPathEntry entries[64];
		int count;
	};
	...
	void        addMusicParticleSystemPath(PlayerMusicManager::ParticleType type, std::string const &path);
	void        addFlourishParticlePath(FlourishParticleSystemPaths &flourishParticleSystemPaths, int index, std::string const &path);
	void        writePlayerMusicStartupTrace(char const *stage);
...
std::string PlayerMusicManagerNamespace::getParticleSystemPath(DataTable const &dataTable, std::string const &name)
{
	...
	int const nameColumn = dataTable.findColumnNumber("name");
	int const pathColumn = dataTable.findColumnNumber("path");

	for (int index = 0; index < rowCount; ++index)
	{
		char const * const particleName = nameColumn >= 0 ? dataTable.getStringValue(nameColumn, index) : 0;

		if (particleName && strcmp(particleName, name.c_str()) == 0)
		{
			char const * const path = pathColumn >= 0 ? dataTable.getStringValue(pathColumn, index) : 0;
			result = path ? path : "";
			break;
		}
	}
...
//-----------------------------------------------------------------------------
void PlayerMusicManagerNamespace::writePlayerMusicStartupTrace(char const *stage)
{
	...
	if (!file)
		return;
	fprintf(file, "PlayerMusicManager:%s\n", stage);
	fclose(file);
}

//-----------------------------------------------------------------------------
void PlayerMusicManagerNamespace::addMusicParticleSystemPath(PlayerMusicManager::ParticleType type, std::string const &path)
{
	if (s_musicParticleSystemPaths.count >= static_cast<int>(sizeof(s_musicParticleSystemPaths.entries) / sizeof(s_musicParticleSystemPaths.entries[0])))
	{
		DEBUG_WARNING(true, ("PlayerMusicManager music particle path table overflow"));
		return;
	}

	MusicParticleSystemPathEntry &entry = s_musicParticleSystemPaths.entries[s_musicParticleSystemPaths.count];
	entry.type = type;
	strncpy(entry.path, path.c_str(), sizeof(entry.path) - 1);
	entry.path[sizeof(entry.path) - 1] = '\0';
	++s_musicParticleSystemPaths.count;
}

//-----------------------------------------------------------------------------
void PlayerMusicManagerNamespace::addFlourishParticlePath(FlourishParticleSystemPaths &flourishParticleSystemPaths, int index, std::string const &path)
{
	if (flourishParticleSystemPaths.count >= static_cast<int>(sizeof(flourishParticleSystemPaths.entries) / sizeof(flourishParticleSystemPaths.entries[0])))
	{
		DEBUG_WARNING(true, ("PlayerMusicManager flourish particle path table overflow"));
		return;
	}

	FlourishParticleSystemPathEntry &entry = flourishParticleSystemPaths.entries[flourishParticleSystemPaths.count];
	entry.index = index;
	strncpy(entry.path, path.c_str(), sizeof(entry.path) - 1);
	entry.path[sizeof(entry.path) - 1] = '\0';
	++flourishParticleSystemPaths.count;
}
...
void PlayerMusicManagerNamespace::loadFlourishParticlePaths(FlourishParticleSystemPaths &flourishParticleSystemPaths, std::string const &path)
{
	...
	if (iff.open(path.c_str(), true))
	{
		...
		for (int row = 0; row < dataTable.getNumRows(); ++row)
		{
			...
			addFlourishParticlePath(flourishParticleSystemPaths, index, path);
		}
	}
	else
	{
...
void PlayerMusicManager::install()
{
	...
	PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("install-entry");
	...
	if (performanceIff.open(performanceFile, true))
	{
		...
		PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("before-performance-load");
		dataTable.load(performanceIff);
		PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("after-performance-load");
		...
		PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("after-performance-rows");
	}
	else
	{
	...
	if (musicParticlesIff.open(musicParticlesFile, true))
	{
		...
		PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("before-music-particles-load");
		dataTable->load(musicParticlesIff);
		PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("after-music-particles-load");

		PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("before-music-loop-boring");
		PlayerMusicManagerNamespace::addMusicParticleSystemPath(PT_musicLoopBoring, PlayerMusicManagerNamespace::getParticleSystemPath(*dataTable, "musicLoopBoring"));
		PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("after-music-loop-boring");
		PlayerMusicManagerNamespace::addMusicParticleSystemPath(PT_musicLoopEntertaining, PlayerMusicManagerNamespace::getParticleSystemPath(*dataTable, "musicLoopEntertaining"));
		PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("after-music-loop-entertaining");
		PlayerMusicManagerNamespace::addMusicParticleSystemPath(PT_musicPaused, PlayerMusicManagerNamespace::getParticleSystemPath(*dataTable, "musicPaused"));
		PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("after-music-paused");
		PlayerMusicManagerNamespace::addMusicParticleSystemPath(PT_musicNotListening, PlayerMusicManagerNamespace::getParticleSystemPath(*dataTable, "musicNotListening"));
		PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("after-music-not-listening");
	}
	else
	{
		DEBUG_WARNING(true, ("Unable to load the player music performance data table: %s", performanceFile));
...

	{
		...
		{
			PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("before-preload");
			...
			for (; iterSoundTemplateNames != soundTemplateFileNames.end(); ++iterSoundTemplateNames)
			{
				...
				char stage[96];
				snprintf(stage, sizeof(stage), "before-preload-%d", preloadIndex);
				PlayerMusicManagerNamespace::writePlayerMusicStartupTrace(stage);

				SoundTemplateList::preload(path);
				snprintf(stage, sizeof(stage), "after-preload-%d", preloadIndex);
				PlayerMusicManagerNamespace::writePlayerMusicStartupTrace(stage);
				++preloadIndex;
			}
			PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("after-preload");
		}
	}
	...
	PlayerMusicManagerNamespace::writePlayerMusicStartupTrace("install-exit");
}
...
std::string PlayerMusicManager::getMusicParticleSystemPath(ParticleType const particleType)
{
	...
	for (int i = 0; i < s_musicParticleSystemPaths.count; ++i)
	{
		if (s_musicParticleSystemPaths.entries[i].type == particleType)
		{
			result = s_musicParticleSystemPaths.entries[i].path;
			break;
		}
	}
...
std::string PlayerMusicManager::getFlourishParticleSystemPath(int const skillMod, int const flourishCount)
{
	...
	FlourishParticleSystemPaths const *flourishParticleSystemPaths = NULL;

	// Determine which list of paths to use

	if ((skillMod < 50) &&
	    (s_flourishLowParticleSystemPaths.count > 0))
	{
		flourishParticleSystemPaths = &s_flourishLowParticleSystemPaths;
	}
	else if ((skillMod < 100) &&
	         (s_flourishMediumParticleSystemPaths.count > 0))
	{
		flourishParticleSystemPaths = &s_flourishMediumParticleSystemPaths;
	}
	else if (s_flourishHighParticleSystemPaths.count > 0)
	{
		flourishParticleSystemPaths = &s_flourishHighParticleSystemPaths;
	}
	...
	if (flourishParticleSystemPaths != NULL)
	{
		...
		for (int i = 0; i < flourishParticleSystemPaths->count; ++i)
		{
			if (flourishCount > flourishParticleSystemPaths->entries[i].index)
			{
				foundIndex = i;
			}
			else
		...
		result = flourishParticleSystemPaths->entries[foundIndex].path;
	}
