...
float PlayerCreatureController::realAlter (const float elapsedTime)
{
	...
	if (shouldProcessMovement())
	{
		...
		bool preserveAnalogMovementMagnitude = false;
		...
		//-- read queue
		{
			...
			for (i = 0; i < getMessageQueue ()->getNumberOfMessages (); i++)
			{
				...
				case CM_moveLateral:
					...
					preserveAnalogMovementMagnitude = true;
					break;

				case CM_moveLongitudinal:
					desiredVelocity_c.z += value;
					preserveAnalogMovementMagnitude = true;
					break;
		...
		else
		{
			...
			if (preserveAnalogMovementMagnitude)
			{
				float const desiredMagnitude = desiredVelocity_c.magnitude();
				if (desiredMagnitude > 1.0f)
					desiredVelocity_c /= desiredMagnitude;
			}
			else
