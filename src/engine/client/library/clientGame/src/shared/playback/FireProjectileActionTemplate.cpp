...
namespace FireProjectileActionTemplateNamespace
{
	...
	float environmentFloat(char const *name, float defaultValue)
	{
		...
		char *end = 0;
		float const parsed = static_cast<float>(strtod(value, &end));
		return end != value ? parsed : defaultValue;
	}

	bool isVrTabCombatTargetingEnabled()
	{
		if (!environmentEnabledDefault("SWG_OG_VR", false) && !environmentEnabledDefault("SWG_D3D11_VR", false))
			return false;

		return environmentEnabledDefault("SWG_OG_VR_TAB_COMBAT_TARGETING", true);
	}

	void traceVrTabProjectile(Object const *targetObject, Vector const &startPosition_w, Vector const &endPosition_w)
	{
		char const * const path = getenv("SWG_OG_VR_PROOF");
		if (!path || !*path)
			return;
		...
		fprintf(file,
			"{\"event\":\"ogVrTabProjectile\",\"target\":\"%s\",\"start\":[%0.3f,%0.3f,%0.3f],\"end\":[%0.3f,%0.3f,%0.3f]}\n",
			targetObject ? targetObject->getNetworkId().getValueString().c_str() : "0",
			startPosition_w.x,
			startPosition_w.y,
			startPosition_w.z,
			endPosition_w.x,
	...
	Vector getVrTabCombatTargetCenter_w(Object const &targetObject)
	{
		AxialBox const targetBox_o = targetObject.getTangibleExtent();
		if (!targetBox_o.isEmpty() && targetBox_o.getHeight() > 0.01f)
		{
			float heightFraction = environmentFloat("SWG_OG_VR_TAB_COMBAT_TARGET_HEIGHT_FRACTION", 0.45f);
			if (heightFraction < 0.05f)
				heightFraction = 0.05f;
			if (heightFraction > 0.95f)
				heightFraction = 0.95f;

			Vector targetCenter_o = targetBox_o.getBase();
			targetCenter_o.y = targetBox_o.getMin().y + targetBox_o.getHeight() * heightFraction;
			return targetObject.rotateTranslate_o2w(targetCenter_o);
		}

		return targetObject.getAppearanceSphereCenter_w();
	}

	bool getVrTabCombatTargetHitPosition_w(Object const &targetObject, Vector const &startPosition_w, Vector &endPosition_w)
	{
		Vector const targetCenter_w = getVrTabCombatTargetCenter_w(targetObject);
		Appearance const * const appearance = targetObject.getAppearance();
		if (appearance && environmentEnabledDefault("SWG_OG_VR_TAB_COMBAT_TARGET_APPEARANCE_COLLIDE", true))
		{
			Vector rayDirection_w = targetCenter_w - startPosition_w;
			if (rayDirection_w.approximateMagnitude() > 0.01f)
			{
				rayDirection_w.approximateNormalize();
				Vector const rayEnd_w = targetCenter_w + rayDirection_w * environmentFloat("SWG_OG_VR_TAB_COMBAT_TARGET_RAY_EXTRA_METERS", 2.0f);
				Vector const start_o = targetObject.rotateTranslate_w2o(startPosition_w);
				Vector const end_o = targetObject.rotateTranslate_w2o(rayEnd_w);
...
bool FireProjectileActionTemplate::update(float deltaTime, PlaybackScript &script) const
{
	...
	bool const vrTabCombatProjectile = Game::getPlayer() == getEmitterObject(script) && isVrTabCombatTargetingEnabled();
	...
	else if (vrTabCombatProjectile && targetObject != 0)
	{
		IGNORE_RETURN(getVrTabCombatTargetHitPosition_w(*targetObject, startPosition_w, endPosition_w));
	}
	...
	if (vrTabCombatProjectile)
		traceVrTabProjectile(targetObject, startPosition_w, endPosition_w);
...
bool FireProjectileActionTemplate::getProjectileStartPosition(PlaybackScript &script, const CellProperty *&startCellProperty, Vector &startPosition_w, CrcLowerString &emitterHardpoint) const
{
	...
	if (Game::getPlayer() == emittingObject && isVrTabCombatTargetingEnabled())
	{
		if (environmentEnabledDefault("SWG_OG_VR_TAB_COMBAT_PROJECTILE_MUZZLE_ORIGIN", true) && VrDetachedHands::getRightWeaponMuzzlePosition_w(emitterHardpoint, startPosition_w, startCellProperty))
			return true;

		WeaponObject const * const weaponObject = dynamic_cast<WeaponObject const *>(getAttackerWeaponObject(script));
		if (weaponObject && getHardpointPositionByName(*weaponObject, emitterHardpoint, startPosition_w))
		{
			startCellProperty = weaponObject->getParentCell() ? weaponObject->getParentCell() : CellProperty::getWorldCellProperty();
			return true;
		}

		Vector startPosition = emittingObject->getCollisionSphereExtent_w().getCenter();
		startPosition.y += environmentFloat("SWG_OG_VR_TAB_COMBAT_PROJECTILE_UP_METERS", 0.15f);
		startPosition += emittingObject->getObjectFrameK_w() * environmentFloat("SWG_OG_VR_TAB_COMBAT_PROJECTILE_FORWARD_METERS", 0.55f);
		startPosition_w = startPosition;
		return true;
	}
