...
namespace CreatureObjectNamespace
{
	...
	bool envFlagDefault(char const *name, bool defaultValue);
	bool shouldHidePlayerBodyForDetachedVrHands(CreatureObject const &creature);
	bool shouldCreatureBeVisible(CreatureObject const &creature);
	bool shouldCreatureMeshBeVisible(CreatureObject const &creature);
	...
	void writeCreatureStartupTrace(char const *stage)
	{
		...
		if (!file)
			return;
		fprintf(file, "CreatureObject:%s\n", stage);
		fclose(file);
	}

	// Changing appearance for droids in space
	struct DroidSpaceAppearanceOverride
	{
		std::string shipName;
		std::string groundAppearance;
		std::string spaceAppearance;
	};
	int const s_maxDroidSpaceAppearanceOverrides = 4096;
	static DroidSpaceAppearanceOverride s_droidSpaceAppearanceOverrides[s_maxDroidSpaceAppearanceOverrides];
	static int s_droidSpaceAppearanceOverrideCount = 0;
	...
	{
		writeCreatureStartupTrace("load-space-appearance-entry");
		s_droidSpaceAppearanceOverrideCount = 0;
		Iff iff;
		...
		DataTable dt;
		dt.load (iff);
		writeCreatureStartupTrace("load-space-appearance-after-table-load");
		iff.close();
		...
		for (int row = 0; row < numRows; ++row)
		{
			...
			{	
				if (s_droidSpaceAppearanceOverrideCount < s_maxDroidSpaceAppearanceOverrides)
				{
					DroidSpaceAppearanceOverride &entry = s_droidSpaceAppearanceOverrides[s_droidSpaceAppearanceOverrideCount];
					entry.shipName = shipName;
					entry.groundAppearance = droidAppearance;
					entry.spaceAppearance = spaceAppearance;
					++s_droidSpaceAppearanceOverrideCount;
				}
				else
					WARNING(true, ("Too many droid space appearance overrides in %s", s_spaceDroidAppearanceMapFilename));
			}
		}

		s_spaceAppearanceMapInitialized = true;
		writeCreatureStartupTrace("load-space-appearance-exit");
	}
	...
	Appearance* getSpaceAppearance(const std::string& shipname, const std::string& groundAppearanceName)
	{
		...
		for (int i = 0; i < s_droidSpaceAppearanceOverrideCount; ++i)
		{
			DroidSpaceAppearanceOverride const &entry = s_droidSpaceAppearanceOverrides[i];
			if (entry.shipName == shipname && entry.groundAppearance == groundAppearanceName)
			{
				spaceAppearanceOverride = AppearanceTemplateList::createAppearance (entry.spaceAppearance.c_str ());
				break;
			}	
		}
...
float CreatureObject::alter (float deltaTime)
{
	...
	VrDetachedHands::update(*this);
	if (shouldHidePlayerBodyForDetachedVrHands(*this))
	{
...
void CreatureObject::containedByModified(NetworkId const &oldValue, NetworkId const &newValue, bool isLocal)
{
	...
	bool const visibleNow = shouldCreatureMeshBeVisible(*this);
	SkeletalAppearance2 * const appearance = getAppearance() ? getAppearance()->asSkeletalAppearance2() : 0;
	if (appearance)
...
void CreatureObject::setAppearanceTransformModifiers()
{
	...
	if (this == Game::getPlayerCreature() && !shouldHidePlayerBodyForDetachedVrHands(*this))
		VrBodyIK::markModifiersDirty();
#endif
...
bool CreatureObjectNamespace::envFlagDefault(char const *name, bool defaultValue)
{
...
bool CreatureObjectNamespace::shouldHidePlayerBodyForDetachedVrHands(CreatureObject const &creature)
{
#if defined(SWG_VR_PHYSICS_CLIENTOBJECT_HOOK)
	if (&creature != Game::getPlayerCreature())
		return false;

	bool const vrEnabled = envFlagDefault("SWG_OG_VR", false) || envFlagDefault("SWG_D3D11_VR", false);
	if (!vrEnabled)
		return false;

	if (!envFlagDefault("SWG_OG_VR_DETACHED_HANDS", true))
		return false;

	return !envFlagDefault("SWG_OG_VR_SHOW_PLAYER_BODY", false);
#else
	UNREF(creature);
	return false;
#endif
...
bool CreatureObjectNamespace::shouldCreatureMeshBeVisible(CreatureObject const &creature)
{
	return shouldCreatureBeVisible(creature) && !shouldHidePlayerBodyForDetachedVrHands(creature);
}
