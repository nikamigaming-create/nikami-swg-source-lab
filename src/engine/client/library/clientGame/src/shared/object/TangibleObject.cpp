...
void TangibleObject::setClientOnlyVrPhysicsHoverEffect(bool const active, char const * const effectFile, float const scale)
{
	static std::string const label("swg_vr_physics_hover");
	if (active)
	{
		if (effectFile && *effectFile)
			AddObjectEffect(label, effectFile, "", Vector::zero, scale);
	}
	else
	{
		RemoveObjectEffect(label);
	}
}
