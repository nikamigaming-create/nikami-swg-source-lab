...
namespace ShipChassisNamespace
{
	...
	void writeStartupTrace(char const *stage, int row = -1)
	{
		...
		char line[256];
		int const count = _snprintf_s(line, sizeof(line), _TRUNCATE, "ShipChassis:%s row=%d\r\n", stage, row);
		if (count > 0)
		{
...
void ShipChassis::install ()
{
	...
	load();
	writeStartupTrace("after-load-call");
...
{	
...
#ifdef _WIN64
	DataTable * const dtStorage = new DataTable;
	DataTable & dt = *dtStorage;
#else
	DataTable dt;
#endif
	writeStartupTrace("before-datatable-load");
	dt.load (iff);
	writeStartupTrace("after-datatable-load");
	iff.close ();
	writeStartupTrace("after-iff-close");
	...
	{
		writeStartupTrace("row-start", row);
		...
		writeStartupTrace("row-after-name", row);
