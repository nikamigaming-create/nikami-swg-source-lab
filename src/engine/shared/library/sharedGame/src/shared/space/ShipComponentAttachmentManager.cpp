...
namespace ShipComponentAttachmentManagerNamespace
{
	...
	void writeStartupTrace(char const *stage, char const *table = 0, int row = -1)
	{
		...
		char line[640];
		int const count = _snprintf_s(line, sizeof(line), _TRUNCATE, "ShipComponentAttachmentManager:%s table=%s row=%d\r\n", stage, table ? table : "", row);
		if (count > 0)
		{
...
void ShipComponentAttachmentManager::load()
{
	...
	for (ShipChassis::PersistentCrcStringVector::const_iterator it = shipChassisNames.begin (); it != shipChassisNames.end (); ++it)
	{
		...
		writeStartupTrace("table-start", chassis_filename.c_str());
		
#ifdef _WIN64
		Iff * const iff = new Iff;
#else
		Iff stackIff;
		Iff * const iff = &stackIff;
#endif
		
		if (!iff->open (chassis_filename.c_str (), true))
		{
			writeStartupTrace("table-missing", chassis_filename.c_str());
			continue;
		}
		
#ifdef _WIN64
		DataTable * const dt = new DataTable;
#else
		DataTable stackDataTable;
		DataTable * const dt = &stackDataTable;
#endif
		writeStartupTrace("before-load", chassis_filename.c_str());
		dt->load (*iff);
		writeStartupTrace("after-load", chassis_filename.c_str());
#ifndef _WIN64
		iff->close ();
#endif
		writeStartupTrace("after-close", chassis_filename.c_str());

		const int numRows = dt->getNumRows ();
		writeStartupTrace("after-row-count", chassis_filename.c_str(), numRows);
		
		for (int row = 0; row < numRows; ++row)
		{
			writeStartupTrace("row-start", chassis_filename.c_str(), row);
			...
			writeStartupTrace("row-after", chassis_filename.c_str(), row);
		}
		writeStartupTrace("table-after-rows", chassis_filename.c_str());
	}
