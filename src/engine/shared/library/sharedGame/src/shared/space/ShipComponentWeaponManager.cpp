...
namespace ShipComponentWeaponManagerNamespace
{
	...
	void writeStartupTrace(char const *stage, int row = -1)
	{
		...
		char line[256];
		int const count = _snprintf_s(line, sizeof(line), _TRUNCATE, "ShipComponentWeaponManager:%s row=%d\r\n", stage, row);
		if (count > 0)
		{
...
void ShipComponentWeaponManager::install()
{
	...
	if (dt == NULL)
	{
		...
		writeStartupTrace("missing-table");
		return;
	}
	...
	for (int row = 0; row < numRows; ++row)
	{
		...
		if (shipComponentDescriptor == NULL)
		{
			...
			writeStartupTrace("row-invalid-component", row);
			continue;
		}
		...
		if (shipComponentDescriptor->getComponentType() != ShipComponentType::SCT_weapon)
		{
			...
			writeStartupTrace("row-non-weapon", row);
			continue;
		}
		writeStartupTrace("row-after-descriptor", row);
		...
		writeStartupTrace("row-after-insert", row);
	}
