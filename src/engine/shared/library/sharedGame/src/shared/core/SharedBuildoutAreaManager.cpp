...
namespace SharedBuildoutAreaManagerNamespace
{
	...
	void writeStartupTrace(char const *stage, char const *name, int row)
	{
#ifdef _WIN32
		char path[MAX_PATH];
		...
		char line[512];
		int const count = _snprintf_s(line, sizeof(line), _TRUNCATE, "SharedBuildoutAreaManager:%s name=%s row=%d\r\n", stage, name ? name : "", row);
		if (count > 0)
		{
...
void SharedBuildoutAreaManager::install()
{
	...
	writeStartupTrace("install-entry", "", -1);
	s_installed = true;
	...
	{
		writeStartupTrace("scenes-before-load", "datatables/buildout/buildout_scenes.iff", -1);
		DataTable buildoutScenesTable;
		buildoutScenesTable.load(iff);
		writeStartupTrace("scenes-after-load", "datatables/buildout/buildout_scenes.iff", -1);

		int const sceneCount = buildoutScenesTable.getNumRows();
		for (int sceneRow = 0; sceneRow < sceneCount; ++sceneRow)
		{
			writeStartupTrace("scene-row-start", "", sceneRow);
			...
			writeStartupTrace("scene-row-after-name", sceneName.c_str(), sceneRow);
			...
			writeStartupTrace("scene-row-after-adjust", sceneName.c_str(), sceneRow);
		}
	}
	writeStartupTrace("after-scenes", "", static_cast<int>(s_buildoutScenes.size()));
  	...
  	for (int i = 0; i < numBuildoutScenes; ++i)
	{
		...
		writeStartupTrace("area-scene-start", buildoutScene.c_str(), i);
  
  		IGNORE_RETURN(snprintf(filename, sizeof(filename)-1, "datatables/buildout/areas_%s.iff", buildoutScene.c_str()));
		writeStartupTrace("area-before-iff-open", filename, i);

		Iff iff;
		if (iff.open(filename, true))
		{
			writeStartupTrace("area-after-iff-open", filename, i);
			DataTable areaListTable;
			writeStartupTrace("area-before-load", filename, i);
			areaListTable.load(iff);
			writeStartupTrace("area-after-load", filename, i);

			std::vector<BuildoutArea> areasForScene;
			writeStartupTrace("area-after-local-vector", filename, i);

			int const areaCount = areaListTable.getNumRows();
			writeStartupTrace("area-before-rows", filename, areaCount);
			for (int areaRow = 0; areaRow < areaCount; ++areaRow)
			{
				writeStartupTrace("area-row-start", filename, areaRow);
				areasForScene.push_back();
				writeStartupTrace("area-row-after-push", filename, areaRow);
				...
				writeStartupTrace("area-row-after-fill", filename, areaRow);
			}

			writeStartupTrace("area-before-map-insert", buildoutScene.c_str(), i);
  			s_buildoutAreas[buildoutScene] = areasForScene;
			writeStartupTrace("area-after-map-insert", buildoutScene.c_str(), i);
		}
		else
		{
			WARNING(true, ("SharedBuildoutAreaManager unable to load areas file [%s]", filename));
			writeStartupTrace("area-iff-missing", filename, i);
		}
	}

	ExitChain::add(remove, "SharedBuildoutAreaManager::remove");
	writeStartupTrace("install-exit", "", -1);
}
