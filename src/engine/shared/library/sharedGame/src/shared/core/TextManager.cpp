...
namespace TextManagerNamespace
{
	...
	void writeStartupTrace(char const *stage)
	{
		...
		char line[256];
		int const count = _snprintf_s(line, sizeof(line), _TRUNCATE, "TextManager:%s\r\n", stage);
		if (count > 0)
		{
...
void TextManager::install()
{
	...
	{
#ifdef _WIN64
		DataTable * const dataTableStorage = new DataTable;
		DataTable & dataTable = *dataTableStorage;
#else
		DataTable dataTable;
#endif
...
#ifdef _WIN64
		profanityFilterIff.setOwnsData(false);
		writeStartupTrace("after-detach-data-x64");
#endif

		int const rowCount = dataTable.getNumRows();
		writeStartupTrace("after-row-count");
	...
	s_installed = true;
	writeStartupTrace("after-install");
}
