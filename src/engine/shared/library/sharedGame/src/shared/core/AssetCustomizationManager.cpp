...
#pragma pack(pop)
	...
	void writeStartupTrace(char const *stage)
	{
		...
		char line[256];
		int const count = _snprintf_s(line, sizeof(line), _TRUNCATE, "AssetCustomizationManager:%s\r\n", stage);
		if (count > 0)
		{
...
void AssetCustomizationManager::install(char const *filename)
{
	...
	writeStartupTrace("install-after-validation");
	...
	writeStartupTrace("install-after-endian-check");

	Iff iff;
	writeStartupTrace("install-before-open");
	bool const openResult = iff.open(filename, true);
	writeStartupTrace(openResult ? "install-after-open" : "install-open-failed");
	...
	writeStartupTrace("install-after-detach-data-x64");
#endif
