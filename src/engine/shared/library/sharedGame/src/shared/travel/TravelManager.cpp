...
namespace TravelManagerNamespace
{
	...
	void writeStartupTraceValue(char const *stage, char const *name, int value);
...
void TravelManagerNamespace::writeStartupTrace(char const *stage)
{
#ifdef _WIN32
	char path[MAX_PATH];
	...
	char line[256];
	int const count = _snprintf_s(line, sizeof(line), _TRUNCATE, "TravelManager:%s\r\n", stage ? stage : "unknown");
	if (count > 0)
	{
...
void TravelManagerNamespace::writeStartupTraceValue(char const *stage, char const *name, int value)
{
#ifdef _WIN32
	char path[MAX_PATH];
	DWORD const result = GetEnvironmentVariable("SWG_STARTUP_TRACE_FILE", path, sizeof(path));
	...
	char line[320];
	int const count = _snprintf_s(line, sizeof(line), _TRUNCATE, "TravelManager:%s name=%s value=%d\r\n", stage ? stage : "unknown", name ? name : "", value);
	if (count > 0)
	{
		DWORD bytesWritten = 0;
		IGNORE_RETURN(WriteFile(file, line, static_cast<DWORD>(strlen(line)), &bytesWritten, NULL));
...
void TravelManager::install ()
{
	...
	writeStartupTrace("install-after-debug-flag");
...
void TravelManagerNamespace::load (const char* fileName)
{
	...
	writeStartupTrace("load-after-clear");

	Iff iff;
	writeStartupTrace("load-after-iff-ctor");
	bool const openSuccess = iff.open (fileName, true);
	writeStartupTraceValue("load-after-open", fileName, openSuccess ? 1 : 0);
	if (openSuccess)
	{
		char tagName[5];
		ConvertTagToString(iff.getCurrentName(), tagName);
		writeStartupTraceValue("load-current-tag", tagName, iff.getRawDataSize());
		DataTable dataTable;
		writeStartupTrace("load-before-data-table-load");
		dataTable.load (iff);
		writeStartupTrace("load-after-data-table-load");
#ifdef _WIN64
		iff.setOwnsData(false);
		writeStartupTrace("load-after-iff-detach-data-x64");
#endif

		//-- setup planet names
		const int numberOfColumns = dataTable.getNumColumns ();
		writeStartupTraceValue("load-column-count", fileName, numberOfColumns);
		int column(0);
