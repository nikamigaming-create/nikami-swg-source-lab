...
namespace FatalNamespace
{
	...
	void appendStartupFatalTrace(char const * message)
	{
		char const * const path = getenv("SWG_STARTUP_TRACE_FILE");
		if (!path || !*path)
			return;
		...
		fputs("Fatal:\n", file);
		fputs(message ? message : "<null fatal message>\n", file);
		fclose(file);
	}
}
...
static void InternalFatal(const char *format, va_list va)
{
	...
	appendStartupFatalTrace(ms_buffer);
