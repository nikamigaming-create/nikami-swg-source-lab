...
namespace
{
	void writeConfigStartupTrace(char const *stage, char const *section = 0, char const *key = 0)
	{
		char const *verbose = getenv("SWG_VERBOSE_STARTUP_TRACE");
		if (!verbose || !*verbose)
			return;
		...
		if (section && key)
			fprintf(file, "ConfigFile:%s section=%s key=%s\n", stage, section, key);
		else
			fprintf(file, "ConfigFile:%s\n", stage);
		fclose(file);
	}
}
...
bool ConfigFile::loadFromCommandLine(const char *buffer)
{
	...
	while (*buffer)
	{
		...
		if (*buffer == '@')
		{
			...
			const int fileLen = static_cast<int>(fileEnd - fileBegin);
		...
		else if (*buffer == '-' && *(buffer+1) == 's' && *(buffer+2) == ' ')
		{
			...
			const int sectionLen = static_cast<int>(sectionEnd - sectionBegin);
		...
		else
		{
			...
			NOT_NULL(keyEnd);
			//make a string out of it
			const int keyLen = static_cast<int>(keyEnd - keyBegin);
			...
			//make a string out of it
			const int valueLen = static_cast<int>(valueEnd - valueBegin);
...
bool ConfigFile::loadFromBuffer(char const * const buffer, int const length)
{
	...
	while(bufferPosition < buffer + length && *bufferPosition)
	{
		...
		lineLength = static_cast<int>(lineEnd - bufferPosition);
...
void ConfigFile::processLine(const char *line)
{
	...
	if (line == strstr(line, ms_IncludeDelimiter))
	{
		...
		if (fileName[0] == '\"')
		{
			...
			NOT_NULL(endQuote);
			const int len = static_cast<int>(endQuote - fileName);
	...
	if (*line == '[')
	{
		...
		NOT_NULL(endSection);
		const int len = static_cast<int>(endSection - line);
...
void ConfigFile::processKeys(const char *line)
{
	...
	while (isspace(*endKey))
		--endKey;
	const int len = static_cast<int>(endKey - line + 1);
	...
	while (*endValue)
	{
		...
		if (*beginValue != '"')
		{
			...
			while(*endValue &&
					...
					(*endValue) != ';')
				++endValue;
			int valueLength = static_cast<int>(endValue - beginValue);
...
{
	writeConfigStartupTrace("get-bool-entry", section, key);
	NO_SPACE_SECTION(section);
	NO_SPACE_KEY(key);
	...
	NOT_NULL(section);
	NOT_NULL(key);
	writeConfigStartupTrace("get-bool-before-section", section, key);
	Section *s = getSection(section);
	writeConfigStartupTrace("get-bool-after-section", section, key);
	if(!s)
	{
		writeConfigStartupTrace("get-bool-before-create-section", section, key);
		s = createSection(section);
		writeConfigStartupTrace("get-bool-after-create-section", section, key);
	}

	writeConfigStartupTrace("get-bool-before-find-key", section, key);
	Key *k = s->findKey(key);
	writeConfigStartupTrace("get-bool-after-find-key", section, key);

	if (k && k->getLazyAdd() && overrideLazyAdds)
	{
		writeConfigStartupTrace("get-bool-before-remove-lazy", section, key);
		writeConfigStartupTrace("get-bool-after-remove-lazy", section, key);
		return defaultValue;
	}

	if(!k)
	{
		writeConfigStartupTrace("get-bool-before-add-key", section, key);
		s->addKey(key, FormattedString<32>().sprintf("%s", defaultValue ? "1" : "0"), !overrideLazyAdds);
		writeConfigStartupTrace("get-bool-after-add-key", section, key);
		return defaultValue;
	}

	writeConfigStartupTrace("get-bool-before-get-key-bool", section, key);
