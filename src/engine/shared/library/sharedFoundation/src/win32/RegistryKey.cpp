...
void RegistryKey::install(const char *productKeyRelativePath)
{
	...
	// Open the product machine key only if it already exists.  Normal user
	// launches should not require HKLM write/create access.
	HKEY machineProductKey = 0;
	LONG const machineProductResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, useRegistryPath, 0, KEY_READ, &machineProductKey);
	if (machineProductResult == ERROR_SUCCESS)
		productMachineKey = new RegistryKey(machineProductKey, true);
	else
		productMachineKey = new RegistryKey(HKEY_LOCAL_MACHINE, false);
