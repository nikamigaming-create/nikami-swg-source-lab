...
#else
	// On x64 the FPU control word is via _controlfp_s
	unsigned int cw = 0;
	_controlfp_s(&cw, 0, 0);
	controlWord = static_cast<WORD>(cw & 0xFFFF);
#endif
	return controlWord;
}
...
void FloatingPointUnit::setControlWord(WORD controlWord)
{
	...
	IGNORE_RETURN(_controlfp_s(&currentControl, _RC_NEAR | _MCW_EM, _MCW_RC | _MCW_EM));
#endif
}
