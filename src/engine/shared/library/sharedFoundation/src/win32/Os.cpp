...
namespace OsNamespace
{
	...
	Os::QueueMouseButtonHookFunction              ms_queueMouseButtonDownHookFunction;
	Os::QueueMouseButtonHookFunction              ms_queueMouseButtonUpHookFunction;
	...
	Os::QueueVrControllerInputHookFunction        ms_queueVrControllerInputHookFunction;
	Os::QueueVrMenuInputHookFunction              ms_queueVrMenuInputHookFunction;
...
void Os::setQueueMouseButtonDownHookFunction(QueueMouseButtonHookFunction queueMouseButtonHookFunction)
{
	ms_queueMouseButtonDownHookFunction = queueMouseButtonHookFunction;
}

// ----------------------------------------------------------------------

void Os::setQueueMouseButtonUpHookFunction(QueueMouseButtonHookFunction queueMouseButtonHookFunction)
{
	ms_queueMouseButtonUpHookFunction = queueMouseButtonHookFunction;
}
...
void Os::setQueueVrControllerInputHookFunction(QueueVrControllerInputHookFunction queueVrControllerInputHookFunction)
{
	ms_queueVrControllerInputHookFunction = queueVrControllerInputHookFunction;
}

//-----------------------------------------------------------------

void Os::setQueueVrMenuInputHookFunction(QueueVrMenuInputHookFunction queueVrMenuInputHookFunction)
{
	ms_queueVrMenuInputHookFunction = queueVrMenuInputHookFunction;
}

//-----------------------------------------------------------------

void Os::queueSetSystemMouseCursorPosition(int x, int y)
{
	OsNamespace::updateMousePosition(x, y);
}

//-----------------------------------------------------------------

void Os::queueMouseButtonDown(int mouse, int mouseButton)
{
	if (ms_queueMouseButtonDownHookFunction)
		ms_queueMouseButtonDownHookFunction(mouse, mouseButton);
}

//-----------------------------------------------------------------

void Os::queueMouseButtonUp(int mouse, int mouseButton)
{
	if (ms_queueMouseButtonUpHookFunction)
		ms_queueMouseButtonUpHookFunction(mouse, mouseButton);
}

//-----------------------------------------------------------------

void Os::queueVrControllerInput(float leftX, float leftY, bool leftActive, float rightX, float rightY, bool rightActive, float headYawRadians, bool headYawValid, float leftHandYawRadians, bool leftHandYawValid, float rightHandYawRadians, bool rightHandYawValid, bool turnModeClickPressed)
{
	if (ms_queueVrControllerInputHookFunction)
		ms_queueVrControllerInputHookFunction(leftX, leftY, leftActive, rightX, rightY, rightActive, headYawRadians, headYawValid, leftHandYawRadians, leftHandYawValid, rightHandYawRadians, rightHandYawValid, turnModeClickPressed);
}

//-----------------------------------------------------------------

void Os::queueVrMenuInput(bool pressed)
{
	if (ms_queueVrMenuInputHookFunction)
		ms_queueVrMenuInputHookFunction(pressed);
}
	...
	switch (uMsg)
	{
		...
		case WM_CHAR:
			...
			if (ms_queueCharacterHookFunction)
			{
				...
				if (!extended)
				{
					int keyCode = static_cast<int>((lParam << 8) >> 24);  // key code is in bits 16-23
...
bool Os::launchBrowser(std::string const & website)
{
	std::string URL("http://");
	URL=website;
	INT_PTR const result = reinterpret_cast<INT_PTR>(ShellExecute(NULL, "open", URL.c_str(), NULL, NULL, SW_SHOWNORMAL));
	return (result > 32);
}
