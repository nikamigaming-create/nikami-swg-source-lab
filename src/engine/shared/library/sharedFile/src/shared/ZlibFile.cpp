...
byte * ZlibFile::decompress()
{
	byte const *compressedBuffer = m_compressedBuffer;
	byte *ownedCompressedCopy = NULL;
	if (m_ownsCompressedBuffer && m_compressedBuffer)
	{
		ownedCompressedCopy = new byte[m_compressedBufferLength];
		memcpy(ownedCompressedCopy, m_compressedBuffer, static_cast<size_t>(m_compressedBufferLength));
		delete [] m_compressedBuffer;
		m_compressedBuffer = NULL;
		m_ownsCompressedBuffer = false;
		compressedBuffer = ownedCompressedCopy;
	}

	byte * uncompressedBuffer = new byte[m_uncompressedLength];

	int const expandedLength = ZlibCompressor().expand(compressedBuffer, m_compressedBufferLength, uncompressedBuffer, m_uncompressedLength);
	delete [] ownedCompressedCopy;

	if (expandedLength != m_uncompressedLength)
	{
		delete [] uncompressedBuffer;
		FATAL(true, ("ZlibFile::decompress failed (%d/%d bytes from %d compressed bytes)", expandedLength, m_uncompressedLength, m_compressedBufferLength));
		return NULL;
	}
