...
namespace AppearanceTemplateListNamespace
{
	...
	void writeStartupTrace(char const *stage, char const *name, Tag tag)
	{
		...
		if (file)
		{
			fprintf(file, "AppearanceTemplateList:%s tag=%s name=%s\n", stage ? stage : "", tagString, name ? name : "");
			fclose(file);
		}
	}
...
const AppearanceTemplate *AppearanceTemplateList::fetch(const char *const fileName, bool & found)
{
	...
	writeStartupTrace("fetch-entry", fileName, 0);
	...
	writeStartupTrace("fetch-after-create", actualFileName, 0);

	//-- up the reference count
	const AppearanceTemplate *const result = fetch(appearanceTemplate);
	writeStartupTrace("fetch-exit", actualFileName, 0);
	return result;
}
...
{
	NOT_NULL(fileName);
	writeStartupTrace("create-entry", fileName, 0);

	TemporaryCrcString actualFileName(fileName, true);
	writeStartupTrace("create-after-temp-crc", actualFileName.getString(), 0);
	...
	if (strstr(fileName, ".apt") != 0)
	{
		...
		writeStartupTrace("create-redirect", iter->second->getString(), 0);
		return create(iter->second->getString());
	}

	//-- search for the appearance in the named list
	{
		writeStartupTrace("create-before-named-find", actualFileName.getString(), 0);
		NamedTemplates::iterator iter = ms_namedTemplates.find(&actualFileName);
		if (iter != ms_namedTemplates.end())
		{
			writeStartupTrace("create-named-found", actualFileName.getString(), 0);
			return iter->second;
		}
		writeStartupTrace("create-after-named-find", actualFileName.getString(), 0);
	}
	...
	{
		writeStartupTrace("create-before-timed-find", actualFileName.getString(), 0);
		...
		if (iter != ms_namedTimedTemplates.end())
		{
			...
			ms_namedTimedTemplates.erase(iter);
			writeStartupTrace("create-timed-found", actualFileName.getString(), 0);
		}
		writeStartupTrace("create-after-timed-find", actualFileName.getString(), 0);
	}
	...
	{
		Iff iff;
		writeStartupTrace("create-before-open", actualFileName.getString(), 0);
		if (!iff.open(actualFileName.getString(), true))
			FATAL(true, ("Could not open appearance file %s", actualFileName.getString()));

		const Tag tag = iff.getCurrentName();
		writeStartupTrace("create-after-open", actualFileName.getString(), tag);
		TagBindingMap::iterator iter = ms_tagBindingMap.find(tag);
		if (iter != ms_tagBindingMap.end())
		{
			writeStartupTrace("create-before-factory", actualFileName.getString(), tag);
			appearanceTemplate = iter->second(actualFileName.getString(), &iff);
			writeStartupTrace("create-after-factory", actualFileName.getString(), tag);
		}
		else
		{
			char tagString[5];
	...
	if (appearanceTemplate)
	{
		writeStartupTrace("create-before-add-named", actualFileName.getString(), 0);
		addNamedAppearanceTemplate(appearanceTemplate);
		writeStartupTrace("create-after-add-named", actualFileName.getString(), 0);
	}
	
	writeStartupTrace("create-exit", actualFileName.getString(), 0);
	return appearanceTemplate;
}
