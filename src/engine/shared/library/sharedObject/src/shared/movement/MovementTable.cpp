...
namespace MovementTableNamespace
{
	...
	void writeStartupTrace(char const *stage, char const *table, int row = -1, int value = -1)
	{
		...
		fprintf(file, "MovementTable:%s table=%s row=%d value=%d\n", stage, table ? table : "", row, value);
		fclose(file);
	}
...
{
	writeStartupTrace("install-entry", stateFileName.c_str());
	DEBUG_FATAL(ms_installed, ("MovementTable already installed"));

	//-- Process state locomotion speed limit file.
	{
		writeStartupTrace("state-limit-before-get", stateFileName.c_str());
		DataTable *table = DataTableManager::getTable(stateFileName, true);
		writeStartupTrace("state-limit-after-get", stateFileName.c_str());
		...
		writeStartupTrace("state-limit-row-count", stateFileName.c_str(), rowCount);

		for (i = 0; i < rowCount; ++i)
		{
			writeStartupTrace("state-limit-row-before", stateFileName.c_str(), i);
			States::Enumerator state = static_cast<States::Enumerator>(table->getIntValue("state", i));
			writeStartupTrace("state-limit-row-after-state", stateFileName.c_str(), i, state);
			ms_stateMoveLimit[state] = static_cast<eLocomotionSpeed>(table->getIntValue("maxSpeedCategory", i));
			writeStartupTrace("state-limit-row-after-speed", stateFileName.c_str(), i, ms_stateMoveLimit[state]);
		}
		writeStartupTrace("state-limit-before-close", stateFileName.c_str());
		DataTableManager::close(stateFileName);
		writeStartupTrace("state-limit-after-close", stateFileName.c_str());
	}

	//-- Process state movement rate modifiers file.
	{
		writeStartupTrace("rate-mod-before-clear", cs_stateRateModifierFilename);
		s_stateMovementRateModifiers.clear();
		writeStartupTrace("rate-mod-after-clear", cs_stateRateModifierFilename);

		// @todo -TRF- get filename from caller.
		char const *filename = cs_stateRateModifierFilename;

		writeStartupTrace("rate-mod-before-get", filename);
		DataTable *table = DataTableManager::getTable(filename, true);
		writeStartupTrace("rate-mod-after-get", filename);
		FATAL(!table, ("Couldn't open state movement rate modifier datatable [%s].", filename));

		int const rowCount = table->getNumRows();
		writeStartupTrace("rate-mod-row-count", filename, rowCount);

		for (int i = 0; i < rowCount; ++i)
		{
			writeStartupTrace("rate-mod-row-before", filename, i);
			States::Enumerator const state        = static_cast<States::Enumerator>(table->getIntValue("state", i));
			writeStartupTrace("rate-mod-row-after-state", filename, i, state);
			float const              rateModifier = table->getFloatValue("movementRateModifier", i);
			writeStartupTrace("rate-mod-row-after-rate", filename, i);

			writeStartupTrace("rate-mod-row-before-insert", filename, i, state);
			IGNORE_RETURN(s_stateMovementRateModifiers.insert(StateFloatMap::value_type(state, rateModifier)));
			writeStartupTrace("rate-mod-row-after-insert", filename, i, state);
		}
		writeStartupTrace("rate-mod-before-close", filename);
		DataTableManager::close(filename);
		writeStartupTrace("rate-mod-after-close", filename);
	}

	writeStartupTrace("install-before-installed-flag", stateFileName.c_str());
	ms_installed = true;
	writeStartupTrace("install-exit", stateFileName.c_str());
}
