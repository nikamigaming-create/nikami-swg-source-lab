...
namespace MemoryManagerNamespace
{
	...
	bool       isPointerInsideSystemAllocation(void const *userPointer);
	bool       isExactAllocatedPointerFromSystemAllocation(void const *userPointer);
...
void MemoryManager::debugReportMap()
{
	for (SystemAllocation * systemAllocation = ms_firstSystemAllocation; systemAllocation; systemAllocation = systemAllocation->getNext())
	{
		...
		int const headerLength = static_cast<int>(strlen(buffer));
...
bool MemoryManagerNamespace::isPointerInsideSystemAllocation(void const *userPointer)
{
	if (!userPointer)
		return false;

	byte const * const address = reinterpret_cast<byte const *>(userPointer);
	for (SystemAllocation *systemAllocation = ms_firstSystemAllocation; systemAllocation; systemAllocation = systemAllocation->getNext())
	{
		byte const * const begin = reinterpret_cast<byte const *>(systemAllocation);
		byte const * const end = begin + systemAllocation->getSize();
...
bool MemoryManagerNamespace::isExactAllocatedPointerFromSystemAllocation(void const *userPointer)
{
	if (!userPointer)
		return false;
	...
	for (SystemAllocation *systemAllocation = ms_firstSystemAllocation; systemAllocation; systemAllocation = systemAllocation->getNext())
	{
		...
		Block * const last = systemAllocation->getLastMemoryBlock();
		for (Block *block = systemAllocation->getFirstMemoryBlock()->getNext(); block && block != last; )
		{
			Block * const next = block->getNext();
			if (next <= block || next > last)
				return false;

			if (!block->isFree())
			{
				byte const * const memory = reinterpret_cast<byte const *>(block) + cms_allocatedBlockSize + cms_guardBandSize;
...
#if defined(_WIN64)
		if (!isPointerInsideSystemAllocation(userPointer))
		{
			ms_criticalSection->leave();
			::free(userPointer);
			return;
		}
		if (!isExactAllocatedPointerFromSystemAllocation(userPointer))
		{
			ms_criticalSection->leave();
			return;
		}
				...
				if (block->isFree())
				{
					...
					if (freePatterns)
					{
						...
						for (int i = 0; i < freeSize; ++i, ++memory)
							...
							{
								corrupt = true;
								DEBUG_REPORT_LOG_PRINT(true, ("corrupted free pattern at position %3d [membase=%p, memaddr=%p] = %02x\n", i, reinterpret_cast<byte *>(block) + cms_freeBlockSize, reinterpret_cast<byte *>(block) + cms_freeBlockSize + i, static_cast<int>(*memory)));
...
void MemoryManagerNamespace::logMessageToFd(char const *message)
{
	if (ms_logMessageFd >= 0)
		_write(ms_logMessageFd, message, static_cast<unsigned int>(strlen(message)));
}
...
bool MemoryManager::reportToFile(char const *, bool)
{
	return false;
}
