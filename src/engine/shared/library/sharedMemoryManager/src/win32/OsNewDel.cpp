...
#else  // !_M_IX86  — x64 C++ operator new/delete without asm trampolines

void *operator new(size_t size, MemoryManagerNotALeak) { return localAllocate(size, 0, false, false); }
void *operator new(size_t size) { return localAllocate(size, 0, false, true); }
void *operator new[](size_t size) { return localAllocate(size, 0, true, true); }
void *operator new(size_t size, const char *, int) { return localAllocate(size, 0, false, true); }
void *operator new[](size_t size, const char *, int) { return localAllocate(size, 0, true, true); }
