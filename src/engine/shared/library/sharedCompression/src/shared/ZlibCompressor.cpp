...
#else
	size_t const totalSize = static_cast<size_t>(items) * static_cast<size_t>(size);
...
#else
	size_t const addressValue = reinterpret_cast<size_t>(address);
	size_t const poolBegin = reinterpret_cast<size_t>(ms_memoryBottom);
	size_t const poolEnd = reinterpret_cast<size_t>(ms_memoryTop);

	if (addressValue < poolBegin || addressValue >= poolEnd)
		operator delete(address);
	else
	{
		ms_mutex.enter();
...
{
#ifdef _WIN64
	byte const *inputBegin = reinterpret_cast<byte const *>(inputBuffer);
	byte const *inputEnd = inputBegin + inputSize;
	byte *outputBegin = reinterpret_cast<byte *>(outputBuffer);
	byte *outputEnd = outputBegin + outputSize;
	bool const inputOutputOverlap =
		reinterpret_cast<size_t>(inputBegin) < reinterpret_cast<size_t>(outputEnd) &&
		reinterpret_cast<size_t>(outputBegin) < reinterpret_cast<size_t>(inputEnd);
	byte *inputCopy = NULL;
	if (inputOutputOverlap)
	{
		inputCopy = new byte[inputSize];
		memcpy(inputCopy, inputBegin, static_cast<size_t>(inputSize));
		inputBegin = inputCopy;
	}

	z_stream z;
	memset(&z, 0, sizeof(z));

	z.next_in = reinterpret_cast<Bytef *>(const_cast<byte *>(inputBegin));
	z.avail_in = static_cast<uInt>(inputSize);
	z.next_out = reinterpret_cast<Bytef *>(outputBuffer);
	z.avail_out = static_cast<uInt>(outputSize);
	z.zalloc = allocateWrapper;
	z.zfree = freeWrapper;
	z.opaque = NULL;
	z.data_type = Z_BINARY;

	int initResult = inflateInit(&z);
	if (initResult != Z_OK)
	{
		delete [] inputCopy;
		return -1;
	...
	int const inflateResult = inflate(&z, Z_FINISH);

	int const size = static_cast<int>(z.total_out);
	int const endResult = inflateEnd(&z);
