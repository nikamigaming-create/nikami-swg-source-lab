...
public:
	...
	public:
		...
		iterator end()
		{
			return (m_data ? m_data : emptyData()) + m_size;
		}
	...
	private:
		...
		void reserve(size_t capacity)
		{
			...
			ValueType * const newData = static_cast<ValueType *>(malloc(sizeof(ValueType) * capacity));
			size_t i;
			for(i = 0; i < m_size; ++i)
				newData[i] = m_data[i];
		...
		static ValueType * emptyData()
		{
			static ValueType emptyValue = 0;
			return &emptyValue;
		}
	...
	typedef PointerList<TransceiverBase *> BasePointerList;
#else
	typedef std::vector<TransceiverBase *> BasePointerList;
#endif
...
protected:
	...
	{
		GlobalReceiverInfo();
		BasePointerList                 receivers;
		BasePointerList                 pendingAdds;
		BasePointerList                 pendingRemoves;
...
private:
#ifdef _WIN64
	typedef PointerList<Transceiver<MessageType, IdentifierType> *> ReceiverList;
	typedef PointerList<Transceiver<MessageType, IdentifierType> *> ListenList;
#else
	typedef std::vector<Transceiver<MessageType, IdentifierType> *> ReceiverList;
	typedef std::set<Transceiver<MessageType, IdentifierType> *> ListenList;
#endif

	ReceiverList localReceivers;
	ListenList listenSet;

	// bookkeeping containers and flags to permit safe usage during
	// message emission
	mutable ReceiverList pendingAdds;
	mutable ReceiverList pendingRemoves;
};
...
inline Transceiver<MessageType, IdentifierType>::~Transceiver()
{
	typename ReceiverList::iterator i;
	typename ListenList::iterator f;
...
inline void Transceiver<MessageType, IdentifierType>::addReceiver(Transceiver<MessageType, IdentifierType> * target)
{
	...
	{
		typename ReceiverList::const_iterator f = std::find(localReceivers.begin(), localReceivers.end(), target);
...
inline void Transceiver<MessageType, IdentifierType>::emitMessage(MessageType message) const
{
	...
	typename ReceiverList::const_iterator i;
	...
	if(! pendingRemoves.empty())
	{
		typename ReceiverList::const_iterator a;;
	...
	{
		BasePointerList::const_iterator addIter;
	...
	if(! info.pendingRemoves.empty())
	{
		BasePointerList::const_iterator removeIter;
		for(removeIter = info.pendingRemoves.begin(); removeIter != info.pendingRemoves.end(); ++removeIter)
		{
			BasePointerList::iterator r = std::find(info.receivers.begin(), info.receivers.end(), *removeIter);
...
inline void Transceiver<MessageType, IdentifierType>::listenForAny()
{
	...
	if(! info.locked)
	{
		BasePointerList::const_iterator f = std::find(info.receivers.begin(), info.receivers.end(), this);
...
inline void Transceiver<MessageType, IdentifierType>::removeReceiver(Transceiver<MessageType, IdentifierType> * target)
{
	...
	else
	{
		...
		// receiver list.
		typename ReceiverList::iterator f = std::find(localReceivers.begin(), localReceivers.end(), target);
		if(f != localReceivers.end())
...
private:
	...
	TransceiverBase::BasePointerList receivers;
};
