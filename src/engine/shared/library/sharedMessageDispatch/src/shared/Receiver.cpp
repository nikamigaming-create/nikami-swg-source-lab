...
{
	typedef std::vector<const Emitter *> Container;
	Container c;
};
...
void Receiver::connectToEmitter(const Emitter & target, const char * const messageTypeName)
{
	...
	for(EmitterTargets::Container::const_iterator i = emitterTargets->c.begin(); i != emitterTargets->c.end(); ++i)
	{
		if(*i == &target)
		{
			return;
	...
	emitterTargets->c.push_back(&target);
}
...
{
	for(EmitterTargets::Container::iterator i = emitterTargets->c.begin(); i != emitterTargets->c.end();)
	{
		if(*i == &target)
		{
			i = emitterTargets->c.erase(i);
		}
		else
		{
			++i;
