...
{
	typedef std::vector<Receiver *> ReceiverSet;
	struct Entry
	{
...
Emitter::~Emitter()
{
	...
	{
		ReceiverList::ReceiverSet & targets = (*i).targets;		
...
{
	ReceiverList::Container::iterator i = receiverList->c.begin();
	for(; i != receiverList->c.end(); ++i)
	{
	...
	{
		ReceiverList::Entry entry;
		entry.messageType = messageType;
		entry.targets.push_back(&target);
		receiverList->c.push_back(entry);
	}
	else
	{
		ReceiverList::ReceiverSet & s = (*i).targets;
		for(ReceiverList::ReceiverSet::const_iterator j = s.begin(); j != s.end(); ++j)
		{
			if(*j == &target)
			{
				return;
...
void Emitter::emitMessage(const MessageBase & message) const
{
	...
	for(; i != receiverList->c.end(); ++i)
	{
		if((*i).messageType == message.getType())
		{
			break;
		}
	}
	if(i != receiverList->c.end())
	{
		const ReceiverList::ReceiverSet targets = (*i).targets;
...
{
	bool result = false;
	ReceiverList::Container::const_iterator i = receiverList->c.begin();
	for(; i != receiverList->c.end(); ++i)
	{
		if((*i).messageType == messageType)
		{
...
void Emitter::receiverDestroyed(const Receiver & target) const
{
	...
	{
		ReceiverList::ReceiverSet & targets = (*i).targets;
		for(ReceiverList::ReceiverSet::iterator j = targets.begin(); j != targets.end();)
		{
			if(*j == &target)
			{
				j = targets.erase(j);
...
void Emitter::removeReceiver(const Receiver & target, const char * const messageTypeName) const
{
	...
	if(i != receiverList->c.end())
	{
		ReceiverList::ReceiverSet & rset = (*i).targets;
		for(ReceiverList::ReceiverSet::iterator j = rset.begin(); j != rset.end();)
		{
			if(*j == &target)
			{
				j = rset.erase(j);
