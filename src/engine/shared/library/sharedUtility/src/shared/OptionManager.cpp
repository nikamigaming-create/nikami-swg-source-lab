...
OptionManager::OptionManager () :
	...
	m_fixedRegisteredOptionCount (0)
{
}
...
void OptionManager::addRegisteredOption (Option const & option)
{
	DEBUG_FATAL(m_fixedRegisteredOptionCount >= cms_maxRegisteredOptions, ("OptionManager fixed registered option table overflow"));
	if (m_fixedRegisteredOptionCount < cms_maxRegisteredOptions)
		m_fixedRegisteredOptions[m_fixedRegisteredOptionCount++] = option;
}
...
{
	writeStartupTrace("register-bool-entry");
	Option option (Option::T_bool);
	option.m_bool    = &variable;
	option.m_section = section;
	option.m_name    = name;
	option.m_version = version;
	writeStartupTrace("register-bool-before-add");
	addRegisteredOption (option);
	writeStartupTrace("register-bool-after-add");

	writeStartupTrace("register-bool-before-find");
	variable = findBool (section, name, variable, version);
	writeStartupTrace("register-bool-after-find");
}
...
{
	writeStartupTrace("find-bool-entry");
	...
	writeStartupTrace("find-bool-after-saved-scan");
	if (section && name && ConfigFile::isInstalled ())
	{
		writeStartupTrace("find-bool-before-config");
		return ConfigFile::getKeyBool (section, name, value, true);
	}

	writeStartupTrace("find-bool-exit-default");
	return value;
}
...
void OptionManager::save (Iff & iff)
{
	iff.insertForm (TAG_OPTN);
		...
		iff.insertForm (TAG_0003);
			...
			for (int i = 0; i < m_fixedRegisteredOptionCount; ++i)
			{
				const Option& option = m_fixedRegisteredOptions[i];
...
void OptionManager::loadVersion(Iff & iff, const int version)
{
	...
	copyOptionListIntersection (*m_savedOptionList);
}
...
//----------------------------------------------------------------------
	...
	void OptionManager::copyOptionListIntersection (const OptionList & src)
	{
		for (OptionList::const_iterator it = src.begin (); it != src.end (); ++it)
		{
			const Option & sopt = *it;
			...
			for (int i = 0; i < m_fixedRegisteredOptionCount; ++i)
			{
				const Option & dopt = m_fixedRegisteredOptions[i];

				if (dopt.m_name    && sopt.m_name    && !strcmp (dopt.m_name, sopt.m_name) &&
					dopt.m_section && sopt.m_section && !strcmp (dopt.m_section, sopt.m_section) &&
					dopt.m_type == sopt.m_type &&
					dopt.m_version == sopt.m_version)
				{
					switch (dopt.m_type)
					{
					case Option::T_bool:
