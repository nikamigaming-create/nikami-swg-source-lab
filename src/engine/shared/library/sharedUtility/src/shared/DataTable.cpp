...
namespace
{
	void writeStartupTrace(char const *stage, char const *table, int row, int column)
	{
#ifdef _WIN32
		if (row >= 0 && (!table || !strstr(table, "ship_weapon_components.iff")))
			return;
		...
		char line[640];
		int const count = _snprintf_s(line, sizeof(line), _TRUNCATE, "DataTable:%s table=%s row=%d col=%d\r\n", stage, table ? table : "", row, column);
		if (count > 0)
		{
...
bool DataTable::doesColumnExist(const std::string & column) const
{
	...
	return findColumnNumber(column) >= 0;
}
...
int DataTable::findColumnNumber(const std::string & column) const
{
	...
	for (ColumnIndexMap::const_iterator it = m_columnIndexMap.begin(); it != m_columnIndexMap.end(); ++it)
	{
		if (it->first == column)
			return it->second;
	}
...
{
	char const * const traceName = iff.getFileName() ? iff.getFileName() : "";
	writeStartupTrace("load-entry", traceName, -1, -1);
	IGNORE_RETURN ( iff.enterForm(m_dataTableIffId, false) );

	const Tag version = iff.getCurrentName();
	if (version == TAG_0000)
	{
		writeStartupTrace("load-version-0000", traceName, -1, -1);
		load_0000(iff);
	}
	else if (version == TAG_0001)
	{
		writeStartupTrace("load-version-0001", traceName, -1, -1);
		load_0001(iff);
	}
	else
	{
	...
	writeStartupTrace("load-after-format", traceName, -1, -1);
	...
	writeStartupTrace("load-after-index-vector", traceName, -1, -1);

	buildColumnIndexMap();
	writeStartupTrace("load-after-column-map", traceName, -1, -1);

	if (NULL != iff.getFileName())
		m_name = iff.getFileName();
	else
		m_name.clear();
	writeStartupTrace("load-exit", m_name.c_str(), -1, -1);
}
...
{
	char const * const traceName = iff.getFileName() ? iff.getFileName() : "";
	writeStartupTrace("0000-entry", traceName, -1, -1);
	IGNORE_RETURN ( iff.enterForm(TAG_0000, false) );

	//load columns
	iff.enterChunk(TAG(C,O,L,S));
	m_numCols = iff.read_int32();	
	writeStartupTrace("0000-cols-count", traceName, -1, m_numCols);
	int i = 0;
	m_columns.reserve(m_numCols);
	std::string tmpString;
	for (i = 0; i < m_numCols; ++i)
	{
		writeStartupTrace("0000-col-before-read", traceName, -1, i);
#ifdef _WIN64
		char columnName[1024];
		columnName[0] = 0;
		iff.read_string(columnName, sizeof(columnName) - 1);
		writeStartupTrace("0000-col-after-read-string", traceName, -1, i);
		m_columns.push_back(columnName);
		writeStartupTrace("0000-col-after-push", traceName, -1, i);
#else
		...
		tmpString.clear();
#endif
		writeStartupTrace("0000-col-after-read", traceName, -1, i);
	}

	iff.exitChunk(TAG(C,O,L,S));	
	writeStartupTrace("0000-after-cols", traceName, -1, -1);
	//load type info
	iff.enterChunk(TAG(T,Y,P,E));
	for (i = 0; i < m_numCols; ++i)
	{
		writeStartupTrace("0000-type-before-read", traceName, -1, i);
		...
		writeStartupTrace("0000-type-after-read", traceName, -1, i);
	}
	iff.exitChunk(TAG(T,Y,P,E));
	writeStartupTrace("0000-after-types", traceName, -1, -1);

	//load rows
	iff.enterChunk(TAG(R,O,W,S));
	m_numRows = iff.read_int32();
	writeStartupTrace("0000-rows-count", traceName, m_numRows, -1);
	...
	{
		writeStartupTrace("0000-row-start", traceName, i, -1);
		for (int j = 0; j < m_numCols; ++j)
		{
			writeStartupTrace("0000-cell-before", traceName, i, j);
			_readCell(iff, j, i);
			writeStartupTrace("0000-cell-after", traceName, i, j);
		}
		writeStartupTrace("0000-row-after", traceName, i, -1);
	}

	iff.exitChunk(TAG(R,O,W,S));
	writeStartupTrace("0000-after-rows", traceName, -1, -1);
	iff.exitForm(TAG_0000, false);
	writeStartupTrace("0000-exit", traceName, -1, -1);
}
...
{
	char const * const traceName = iff.getFileName() ? iff.getFileName() : "";
	writeStartupTrace("0001-entry", traceName, -1, -1);
	IGNORE_RETURN( iff.enterForm(TAG_0001, false) );

	//load columns
	iff.enterChunk(TAG(C,O,L,S));
	m_numCols = iff.read_int32();	
	writeStartupTrace("0001-cols-count", traceName, -1, m_numCols);
	int i = 0;
	m_columns.reserve(m_numCols);
	std::string tmpString;
	for (i = 0; i < m_numCols; ++i)
	{
		writeStartupTrace("0001-col-before-read", traceName, -1, i);
#ifdef _WIN64
		char columnName[1024];
		columnName[0] = 0;
		iff.read_string(columnName, sizeof(columnName) - 1);
		writeStartupTrace("0001-col-after-read-string", traceName, -1, i);
		m_columns.push_back(columnName);
		writeStartupTrace("0001-col-after-push", traceName, -1, i);
#else
		iff.read_string(tmpString);
		writeStartupTrace("0001-col-after-read-string", traceName, -1, i);
		m_columns.push_back(tmpString);
		...
		tmpString.clear();
#endif
		writeStartupTrace("0001-col-after-read", traceName, -1, i);
	}

	iff.exitChunk(TAG(C,O,L,S));	
	writeStartupTrace("0001-after-cols", traceName, -1, -1);
	//load type info
	iff.enterChunk(TAG(T,Y,P,E));
	for (i = 0; i < m_numCols; ++i)
	{
		// version 0001 has a format string for the type
		writeStartupTrace("0001-type-before-read", traceName, -1, i);
		m_types.push_back(new DataTableColumnType(iff.read_stdstring()));
		writeStartupTrace("0001-type-after-read", traceName, -1, i);
	}
	iff.exitChunk(TAG(T,Y,P,E));
	writeStartupTrace("0001-after-types", traceName, -1, -1);

	//load rows
	iff.enterChunk(TAG(R,O,W,S));
	m_numRows = iff.read_int32();
	writeStartupTrace("0001-rows-count", traceName, m_numRows, -1);
	...
	for (i = 0; i < m_numRows; ++i)
	{
		writeStartupTrace("0001-row-start", traceName, i, -1);
		for (int j = 0; j < m_numCols; ++j)
		{
			writeStartupTrace("0001-cell-before", traceName, i, j);
			_readCell(iff, j, i);
			writeStartupTrace("0001-cell-after", traceName, i, j);
		}
		writeStartupTrace("0001-row-after", traceName, i, -1);
	}

	iff.exitChunk(TAG(R,O,W,S));
	writeStartupTrace("0001-after-rows", traceName, -1, -1);
	iff.exitForm(TAG_0001, false);
	writeStartupTrace("0001-exit", traceName, -1, -1);
}
...
void DataTable::buildColumnIndexMap()
{
	...
	{
		m_columnIndexMap.push_back(std::make_pair(*i, columnIndex));
	}
}
